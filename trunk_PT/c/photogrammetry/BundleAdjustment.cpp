...
//Professor Nunes mandou matar a itera̤ao sobre os residuos para evitar os valores "NANs"
...
{
	Matrix q1=M12*M22.inverse();

	Matrix q2=q1*M12.transpose();
	Matrix q3=q1*m2;

	Matrix q4=M11-q2;
	Matrix q5=m1-q3;

	return q4.inverse()*q5;
	...
	for (int i=0;i<numImages;i++)
	{
		...
		for (int j=0;j<pnts;j++)
		{
			if (whereInPoints(getPointFrom(i,j))!=-1 && !isCheckingPoint(i,j))
			{
				...
				double y=aux.getY();
				double z=aux.getZ();
				/** Pegar esse metodo na classe do Rafael, as coordenadas pela equa̤ao de colinearidade*/
	...
	for (int i=0;i<numImages;i++)
	{
		...
		for (int j=0; j<numPoints;j++)
		{
			if (whereInPoints(getPointFrom(i,j))!=-1)
			{
				...
				result += " col: ";
				result += intToString(col);
				result += " lin: ";
				result += intToString(lin);
				result += "\n";
			}
