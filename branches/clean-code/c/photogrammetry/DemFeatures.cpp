...
int DemFeatures::exportShpFeatures(char* filename) {
    ...
    for (size_t k = 0; k < feature_classes.size(); k++) {
        bool flag = false;
        ...
        for (unsigned int i = 0; i < features.size(); i++) {
            ...
            if ( featureClassId >= 0
                    &&  static_cast<unsigned int>(featureClassId) == k + 1) {
                ...
                // Cria objeto
                SHPObject* obj = SHPCreateSimpleObject(nShapeType, nVertices, padfX, padfY,
                                                       padfZ );
...
int DemFeatures::getNearestPoint(int fid, double X, double Y, double Z) {
    ...
    for (unsigned int i = 1; i < features.at(fid - 1).points.size(); i++) {
        double dist = sqrt(pow(X - features.at(fid - 1).points.at(i).X,
...
void DemFeatures::convertClassesIdsFromSp165() {
    for (size_t i = 0; i < features.size(); i++) {
        int ft = features.at(i).feature_type;
        int fc = features.at(i).feature_class;
...
int DemFeatures::loadFeatSp165(char* filename, bool append = false) {
    ...
    const int p_feat = features.size();
    ...
    while (!arq.fail()) {
        ...
        int feature_id = atoi(tag.c_str());
        feature_id += p_feat;
        getline(arq, tag); // 2nd line point_id
        int point_id = atoi(tag.c_str());
...
Matrix DemFeatures::createPolygonMap(double Xi, double Yi, double Xf, double Yf,
    ...
    Matrix polyMap;
    // Calculate DEM size
    const int dem_width = int(1.0 + floor((Xf - Xi) / res_x));
    const int dem_height = int(1.0 + floor((Yf - Yi) / res_y));
...
Matrix DemFeatures::mapPolygon(int feat_id, double res_x, double res_y) {
    ...
    const double D0 = sqrt(pow(Xf - Xi, 2) + pow(Yf - Yi, 2));
    // Resize map
    const int pmap_width = int(1.0 + floor((Xf - Xi) / res_x));
    const int pmap_height = int(1.0 + floor((Yf - Yi) / res_y));
...
double DemFeatures::interpolateXYPolygon(int feat_id, double X, double Y,
    ...
    for (size_t i = 0; i < df->points.size(); i++) {
        double D = sqrt(pow(X - df->points.at(i).X, 2) + pow(Y - df->points.at(i).Y,
                        2));
...
double DemFeatures::angle2D(double X1, double Y1, double X2, double Y2) {
    const double theta1 = atan2(Y1, X1);
    const double theta2 = atan2(Y2, X2);
...
bool DemFeatures::isInside(int feat_id, double X, double Y) {
    ...
    double angle = 0.0;
    DemFeature df =  features.at(feat_id - 1);
    const unsigned int n = df.points.size();
