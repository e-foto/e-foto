...
BundleAdjustment::BundleAdjustment(std::deque<Image*>listSelectedImages,
    ...
    calibrated_focal_distance_ = listImages.at(0)->getSensor()->getFocalDistance();
...
void BundleAdjustment::fillDetectorCoordinates() {
    for (int i = 0; i < numImages; i++) {
        ...
        for (int j = 0; j < pnts; j++) {
            double col = getPointFrom(i,
                                      j)->getImageCoordinate(listImages.at(i)->getId()).getCol();
            double lin = getPointFrom(i,
...
//Professor Nunes mandou matar a iteração sobre os residuos para evitar os valores "NANs"
...
void BundleAdjustment::setx1(Matrix N12, Matrix n1) {
    ...
    x1 = SparseMatrix((n11 - temp1 * n12.transpose()).osuInverse()) *
         (n1 - temp1 * n2);
#endif
}
...
Matrix BundleAdjustment::getCoordinatesEqColin(const double X, const double Y,
        const double Z,
        const int imageId) {
    Matrix coord(1, 2);
    const double X0 = getXAdjus(imageId);
    const double Y0 = getYAdjus(imageId);
    const double Z0 = getZAdjus(imageId);
    setRot(getOmegaAdjus(imageId), getPhiAdjus(imageId), getKappaAdjus(imageId));
    const double xi = xsi0 - calibrated_focal_distance_ * (r11 * (X - X0) + r21 *
                      (Y - Y0) + r31 * (Z - Z0)) / (r13 * (X - X0) + r23 * (Y - Y0) + r33 * (Z - Z0));
    const double eta = eta0 - calibrated_focal_distance_ * (r12 * (X - X0) + r22 *
                       (Y - Y0) + r32 * (Z - Z0)) / (r13 * (X - X0) + r23 * (Y - Y0) + r33 * (Z - Z0));
    coord.set(1, 1, xi);
    coord.set(1, 2, eta);
    return coord;
}

Matrix BundleAdjustment::getJacobianaControl(const double X, const double Y,
        const double Z,
        const int imageIndex) {
    Matrix JacCtrl(2, 6);
    const double omega = matAdjust.get(imageIndex, 1);
    const double phi = matAdjust.get(imageIndex, 2);
    const double kappa = matAdjust.get(imageIndex, 3);
    const double X0 = matAdjust.get(imageIndex, 4);
    const double Y0 = matAdjust.get(imageIndex, 5);
    const double Z0 = matAdjust.get(imageIndex, 6);
    JacCtrl.set(1, 1, calibrated_focal_distance_ * cos(kappa)*cos(phi)
                / (sin(phi) * (X - X0) - cos(phi)*sin(omega)
                   * (Y - Y0) + cos(phi)*cos(omega) * (Z - Z0))
                - calibrated_focal_distance_ * (cos(kappa)*cos(phi) * (X - X0)
                        + (sin(kappa)*cos(omega) + cos(kappa)*sin(phi)*sin(omega))
                        * (Y - Y0) + (sin(kappa)*sin(omega)
                                      - cos(kappa)*sin(phi)*cos(omega)) * (Z - Z0))
                / pow((sin(phi) * (X - X0) - cos(phi)*sin(omega) * (Y - Y0)
                       + cos(phi)*cos(omega) * (Z - Z0)), 2)
                *sin(phi));
    JacCtrl.set(1, 2, -calibrated_focal_distance_
                * (-sin(kappa)*cos(omega) - cos(kappa)*sin(phi)*sin(omega))
                / (sin(phi) * (X - X0) - cos(phi)*sin(omega) * (Y - Y0)
                   + cos(phi)*cos(omega) * (Z - Z0))
                + calibrated_focal_distance_ * (cos(kappa)*cos(phi) * (X - X0)
                        + (sin(kappa)*cos(omega) + cos(kappa)
                           *sin(phi)*sin(omega)) * (Y - Y0)
                        + (sin(kappa)*sin(omega) - cos(kappa)
                           *sin(phi)*cos(omega)) * (Z - Z0))
                / pow((sin(phi) * (X - X0) - cos(phi)*sin(omega) * (Y - Y0)
                       + cos(phi)*cos(omega) * (Z - Z0)), 2)
                *cos(phi)*sin(omega));
    JacCtrl.set(1, 3, -calibrated_focal_distance_
                * (-sin(kappa)*sin(omega) + cos(kappa)*sin(phi)*cos(omega))
                / (sin(phi) * (X - X0) - cos(phi)*sin(omega) * (Y - Y0)
                   + cos(phi)*cos(omega) * (Z - Z0))
                - calibrated_focal_distance_
                * (cos(kappa)*cos(phi) * (X - X0)
                ...
                / pow((sin(phi) * (X - X0) - cos(phi)*sin(omega) * (Y - Y0)
                       + cos(phi)*cos(omega) * (Z - Z0)), 2)
                *cos(phi)*cos(omega));
    JacCtrl.set(1, 4, -calibrated_focal_distance_
                * ((-sin(kappa)*sin(omega) + cos(kappa)*sin(phi)*cos(omega))
                   * (Y - Y0) + (sin(kappa)*cos(omega) + cos(kappa)*sin(phi)*sin(omega)) *
                   (Z - Z0))
                / (sin(phi) * (X - X0) - cos(phi)*sin(omega) * (Y - Y0)
                   + cos(phi)*cos(omega) * (Z - Z0))
                + calibrated_focal_distance_
                * (cos(kappa)*cos(phi) * (X - X0) + (sin(kappa)*cos(omega)
                        + cos(kappa)*sin(phi)*sin(omega))
                   * (Y - Y0) + (sin(kappa)*sin(omega) - cos(kappa)*sin(phi)*cos(omega)) *
                   (Z - Z0))
                / pow((sin(phi) * (X - X0) - cos(phi)*sin(omega) * (Y - Y0)
                       + cos(phi)*cos(omega) * (Z - Z0)), 2)
                * (-cos(phi)*cos(omega) * (Y - Y0) - cos(phi)*sin(omega) * (Z - Z0)));
    JacCtrl.set(1, 5, -calibrated_focal_distance_
                * (-cos(kappa)*sin(phi) * (X - X0) + cos(kappa)*cos(phi)*sin(omega) * (Y - Y0)
                ...
                * (cos(phi) * (X - X0) + sin(phi)*sin(omega) * (Y - Y0)
                   - sin(phi)*cos(omega) * (Z - Z0)));
    JacCtrl.set(1, 6, -calibrated_focal_distance_
                * (-sin(kappa)*cos(phi) * (X - X0) + (cos(kappa)*cos(omega)
                        - sin(kappa)*sin(phi)*sin(omega))
                   * (Y - Y0) + (cos(kappa)*sin(omega) + sin(kappa)*sin(phi)*cos(omega)) *
                   (Z - Z0))
                / (sin(phi) * (X - X0) - cos(phi)*sin(omega) * (Y - Y0)
                   + cos(phi)*cos(omega) * (Z - Z0)));
    JacCtrl.set(2, 1, -calibrated_focal_distance_
                *sin(kappa)*cos(phi) / (sin(phi) * (X - X0) - cos(phi)*sin(omega) * (Y - Y0)
                                        + cos(phi)*cos(omega) * (Z - Z0))
                - calibrated_focal_distance_
                * (-sin(kappa)*cos(phi) * (X - X0) + (cos(kappa)*cos(omega)
                        - sin(kappa)*sin(phi)*sin(omega))
                ...
                / pow((sin(phi) * (X - X0) - cos(phi)*sin(omega) * (Y - Y0)
                       + cos(phi)*cos(omega) * (Z - Z0)), 2)
                *sin(phi));
    JacCtrl.set(2, 2, -calibrated_focal_distance_
                * (-cos(kappa)*cos(omega) + sin(kappa)*sin(phi)*sin(omega))
                / (sin(phi) * (X - X0) - cos(phi)*sin(omega) * (Y - Y0)
                   + cos(phi)*cos(omega) * (Z - Z0))
                + calibrated_focal_distance_ * (-sin(kappa)*cos(phi) * (X - X0)
                        + (cos(kappa)*cos(omega) - sin(kappa)*sin(phi)*sin(omega))
                        * (Y - Y0) + (cos(kappa)*sin(omega) + sin(kappa)
                                      *sin(phi)*cos(omega)) * (Z - Z0))
                / pow((sin(phi) * (X - X0) - cos(phi)*sin(omega) * (Y - Y0)
                       + cos(phi)*cos(omega) * (Z - Z0)), 2)
                *cos(phi)*sin(omega));
    JacCtrl.set(2, 3, -calibrated_focal_distance_ *
                ...
                * (-sin(kappa)*cos(phi) * (X - X0) + (cos(kappa)*cos(omega)
                ...
                / pow((sin(phi) * (X - X0) - cos(phi)*sin(omega) * (Y - Y0)
                       + cos(phi)*cos(omega) * (Z - Z0)), 2)
                *cos(phi)*cos(omega));
    JacCtrl.set(2, 4, -calibrated_focal_distance_
                * ((-cos(kappa)*sin(omega) - sin(kappa)*sin(phi)*cos(omega))
                   * (Y - Y0) + (cos(kappa)*cos(omega) - sin(kappa)*sin(phi)*sin(omega)) *
                   (Z - Z0))
                / (sin(phi) * (X - X0) - cos(phi)*sin(omega) * (Y - Y0)
                   + cos(phi)*cos(omega) * (Z - Z0))
                + calibrated_focal_distance_
                * (-sin(kappa)*cos(phi) * (X - X0) + (cos(kappa)*cos(omega)
                ...
                / pow((sin(phi) * (X - X0) - cos(phi)*sin(omega) * (Y - Y0)
                       + cos(phi)*cos(omega) * (Z - Z0)), 2)
                * (-cos(phi)*cos(omega) * (Y - Y0) - cos(phi)*sin(omega) * (Z - Z0)));
    JacCtrl.set(2, 5, -calibrated_focal_distance_
                * (sin(kappa)*sin(phi) * (X - X0) - sin(kappa)*cos(phi)*sin(omega) * (Y - Y0)
                ...
                * (cos(phi) * (X - X0) + sin(phi)*sin(omega) * (Y - Y0)
                   - sin(phi)*cos(omega) * (Z - Z0)));
    JacCtrl.set(2, 6, -calibrated_focal_distance_
                * (-cos(kappa)*cos(phi) * (X - X0)
                   + (-sin(kappa)*cos(omega) - cos(kappa)*sin(phi)*sin(omega))
                   * (Y - Y0) + (-sin(kappa)*sin(omega) + cos(kappa)*sin(phi)*cos(omega))
                   * (Z - Z0)) / (sin(phi) * (X - X0) - cos(phi)*sin(omega) * (Y - Y0)
                                  + cos(phi)*cos(omega) * (Z - Z0)));
    return JacCtrl;
}

Matrix BundleAdjustment::getJacobianaFotogrametric(const double X,
    ...
    const double Y0 = matAdjust.get(imageIndex, 5);
    const double Z0 = matAdjust.get(imageIndex, 6);
    JacFoto.set(1, 1, -calibrated_focal_distance_
                * cos(kappa)*cos(phi) / (sin(phi) * (X - X0)
                                         - cos(phi)*sin(omega) * (Y - Y0) + cos(phi)*cos(omega) *
                                         (Z - Z0))
                + calibrated_focal_distance_ * (cos(kappa)*cos(phi) * (X - X0)
                        + (sin(kappa)*cos(omega)
                           + cos(kappa)*sin(phi)*sin(omega))
                        * (Y - Y0) + (sin(kappa)*sin(omega)
                                      - cos(kappa)*sin(phi)*cos(omega))
                        * (Z - Z0)) / pow(sin(phi)
                                          * (X - X0) - cos(phi)*sin(omega)
                                          * (Y - Y0) + cos(phi)*cos(omega)
                                          * (Z - Z0), 2)*sin(phi));
    JacFoto.set(1, 2, -calibrated_focal_distance_
                * (sin(kappa)*cos(omega) + cos(kappa)*sin(phi)*sin(omega))
                / (sin(phi) * (X - X0) - cos(phi)*sin(omega)
                   * (Y - Y0) + cos(phi)*cos(omega) * (Z - Z0))
                - calibrated_focal_distance_
                * (cos(kappa)*cos(phi) * (X - X0)
                   + (sin(kappa)*cos(omega) + cos(kappa)*sin(phi)*sin(omega)) * (Y - Y0)
                   + (sin(kappa)*sin(omega) - cos(kappa)*sin(phi)*cos(omega)) * (Z - Z0))
                / pow(sin(phi) * (X - X0) - cos(phi)*sin(omega) * (Y - Y0)
                      + cos(phi)*cos(omega) * (Z - Z0), 2)
                *cos(phi)*sin(omega));
    JacFoto.set(1, 3, -calibrated_focal_distance_
                * (sin(kappa)*sin(omega) - cos(kappa)*sin(phi)*cos(omega))
                / (sin(phi) * (X - X0) - cos(phi)*sin(omega)
                   * (Y - Y0) + cos(phi)*cos(omega) * (Z - Z0))
                + calibrated_focal_distance_
                * (cos(kappa)*cos(phi) * (X - X0)
                   + (sin(kappa)*cos(omega) + cos(kappa)*sin(phi)*sin(omega)) * (Y - Y0)
                   + (sin(kappa)*sin(omega) - cos(kappa)*sin(phi)*cos(omega)) * (Z - Z0))
                / pow(sin(phi) * (X - X0) - cos(phi)*sin(omega) * (Y - Y0) + cos(phi)*cos(omega)
                      * (Z - Z0), 2)*cos(phi)*cos(omega));
    JacFoto.set(2, 1, calibrated_focal_distance_ * sin(kappa)*cos(phi)
                / (sin(phi) * (X - X0) - cos(phi)*sin(omega) * (Y - Y0) + cos(phi)*cos(omega) *
                   (Z - Z0))
                + calibrated_focal_distance_
                * (-sin(kappa)*cos(phi) * (X - X0) + (cos(kappa)*cos(omega)
                        - sin(kappa)*sin(phi)*sin(omega))
                   * (Y - Y0) + (cos(kappa)*sin(omega) + sin(kappa)*sin(phi)*cos(omega))
                   * (Z - Z0)) / pow(sin(phi) * (X - X0) - cos(phi)*sin(omega) * (Y - Y0)
                                     + cos(phi)*cos(omega) * (Z - Z0), 2)*sin(phi));
    JacFoto.set(2, 2, -calibrated_focal_distance_ * (cos(kappa)*cos(omega) - sin(
                    kappa)*sin(phi)*sin(omega)) / (sin(phi) * (X - X0) - cos(phi)*sin(omega) *
                            (Y - Y0) + cos(phi)*cos(omega) * (Z - Z0))
                - calibrated_focal_distance_ * (-sin(kappa)*cos(phi) * (X - X0)
                        + (cos(kappa)*cos(omega)
                           - sin(kappa)*sin(phi)*sin(omega)) * (Y - Y0)
                        + (cos(kappa)*sin(omega)
                           + sin(kappa)*sin(phi)*cos(omega)) * (Z - Z0))
                / pow(sin(phi) * (X - X0) - cos(phi)*sin(omega) * (Y - Y0)
                      + cos(phi)*cos(omega) * (Z - Z0), 2)*cos(phi)*sin(omega));
    JacFoto.set(2, 3, -calibrated_focal_distance_ * (cos(kappa)*sin(omega) + sin(
                    kappa)*sin(phi)*cos(omega)) / (sin(phi) * (X - X0) - cos(phi)*sin(omega) *
                            (Y - Y0) + cos(phi)*cos(omega) * (Z - Z0))
                + calibrated_focal_distance_ * (-sin(kappa)*cos(phi) * (X - X0)
                        + (cos(kappa)*cos(omega)
                           - sin(kappa)*sin(phi)*sin(omega)) * (Y - Y0)
                        + (cos(kappa)*sin(omega)
                           + sin(kappa)*sin(phi)*cos(omega)) * (Z - Z0))
                / pow(sin(phi) * (X - X0) - cos(phi)*sin(omega) * (Y - Y0)
                      + cos(phi)*cos(omega) * (Z - Z0), 2)*cos(phi)*cos(omega));
    return JacFoto;
}

//retorna os parametros da imagem numero imageID
Matrix BundleAdjustment::getPTA(Matrix PAf, const int imageId) {
    Matrix result(6, 1);
...
double BundleAdjustment::getdOmegax1(const int imageId) const {
    return x1.get(6 * (imageId - 1) + 4, 1);
}

double BundleAdjustment::getdPhix1(const int imageId) const {
    return x1.get(6 * (imageId - 1) + 5, 1);
}

double BundleAdjustment::getdKappax1(const int imageId) const {
    return x1.get(6 * (imageId - 1) + 6, 1);
}

double BundleAdjustment::getdXx1(const int imageId) const {
    return x1.get(6 * (imageId - 1) + 1, 1);
}

double BundleAdjustment::getdYx1(const int imageId) const {
    return x1.get(6 * (imageId - 1) + 2, 1);
}

double BundleAdjustment::getdZx1(const int imageId) const {
    return x1.get(6 * (imageId - 1) + 3, 1);
}

double BundleAdjustment::getdXx2(const int fotogrPointId) const {
    return x2.get(3 * (fotogrPointId - 1) + 1, 1);
}

double BundleAdjustment::getdYx2(const int fotogrPointId) const {
    return x2.get(3 * (fotogrPointId - 1) + 2, 1);
}

double BundleAdjustment::getdZx2(const int fotogrPointId) const {
    return x2.get(3 * (fotogrPointId - 1) + 3, 1);
}

double BundleAdjustment::getOmegaAdjus(const int imageId) const {
    return matAdjust.get(imageId, 1);
}

double BundleAdjustment::getPhiAdjus(const int imageId) const {
    return matAdjust.get(imageId, 2);
}

double BundleAdjustment::getKappaAdjus(const int imageId) const {
    return matAdjust.get(imageId, 3);
}

double BundleAdjustment::getXAdjus(const int imageId) const {
    return matAdjust.get(imageId, 4);
}

double BundleAdjustment::getYAdjus(const int imageId) const {
    return matAdjust.get(imageId, 5);
}

double BundleAdjustment::getZAdjus(const int imageId) const {
...
Matrix BundleAdjustment::getCoordColinearTerrain(double xsi, double eta,
    ...
    Matrix result(1, 2);
    setRot(getOmegaAdjus(imageId), getPhiAdjus(imageId), getKappaAdjus(imageId));
    const double x = getXAdjus(imageId) + (z - getZAdjus(imageId)) * (r11 *
                     (xsi - xsi0) + r12 * (eta - eta0) - r13 * calibrated_focal_distance_) / (r31 *
                             (xsi - xsi0) + r32 * (eta - eta0) - r33 * calibrated_focal_distance_);
    const double y = getYAdjus(imageId) + (z - getZAdjus(imageId)) * (r21 *
                     (xsi - xsi0) + r22 * (eta - eta0) - r23 * calibrated_focal_distance_) / (r31 *
                             (xsi - xsi0) + r32 * (eta - eta0) - r33 * calibrated_focal_distance_);
    result.set(1, 1, x);
    result.set(1, 2, y);
    return result;
...
void BundleAdjustment::calculateInicialsValues() {
    if (numFotogrametricPoints != 0) {
        ...
        for (int i = 0; i < numImages; i++) {
            ...
            calibrated_focal_distance_ = listImages.at(i)->getSensor()->getFocalDistance();
            ...
            matInitialValues.set(i + 1, 6,
                                 calibrated_focal_distance_ * listImages.at(i)->getFlight()->getScaleDen() /
                                 1000);
        }
    } else {
        Matrix L = createL();
...
Matrix BundleAdjustment::imageToMatrixDetectorCoordenates(Image* img) {
    const int numpnts = img->countPoints();
    Matrix result(0, 6);
...
Matrix BundleAdjustment::digitalToAnalog(InteriorOrientation* oi, double linha,
        double coluna) {
    const double a0 = oi->getXa().get(1, 1);
    const double a1 = oi->getXa().get(2, 1);
    const double a2 = oi->getXa().get(3, 1);
    const double b0 = oi->getXa().get(4, 1);
    const double b1 = oi->getXa().get(5, 1);
    const double b2 = oi->getXa().get(6, 1);
    Matrix result(1, 2);
...
int BundleAdjustment::numberControlPoints(Image* img) {
    int cont = 0;
    const int points = img->countPoints();
...
void BundleAdjustment::zeroingCoordinatesPhotogrammetrics() {
    for (size_t i = 0; i < listPhotogrammetricPoints.size(); i++) {
...
Matrix BundleAdjustment::imageToMatrixJacobiana(int indexImage) {
    ...
    const int pnts = img->countPoints();
    Matrix result(0, 6);
...
void BundleAdjustment::createL0() {
    ...
    for (int i = 0; i < numImages; i++) {
        const int pnts = listImages.at(i)->countPoints();
        ...
        for (int j = 0; j < pnts; j++) {
            if (whereInPoints(getPointFrom(i, j)) != -1 && !isCheckingPoint(i, j)) {
                ...
                double y = aux.getY();
                double z = aux.getZ();
                /** Pegar esse metodo na classe do Rafael, as coordenadas pela equação de colinearidade*/
...
/* setters and getters*/
Matrix BundleAdjustment::getMatRes() const {
    return matRes;
}

std::deque<double> BundleAdjustment::getListRMSE() const {
    return listRMSE;
}

int BundleAdjustment::getTotalIterations() const {
    return totalIterations;
}

Matrix BundleAdjustment::getAFP() const {
    return adjustFinalMat_;
}
...
int BundleAdjustment::testConverged() {
    const int rowsX1 = x1.getRows();
    const int rowsX2 = x2.getRows();
    ...
    for (int i = 0; i < rowsX1; i++) {
        ...
        if (i % 6 < 3) { //Os 3 primeiros elementos são Xo,Yo,Zo
            if (fabs(double(x1.get(i + 1, 1) > metricConvergency))) {
                return 0;
            }
        } else { // Os 3 ultimos elementos são omega, phi, kappa
...
bool BundleAdjustment::isPossibleCalculate() const {
...
double BundleAdjustment::getMetricConvergencyValue() const {
...
double BundleAdjustment::getAngularConvergencyValue() const {
...
Matrix BundleAdjustment::getMatrixInicialValues() const {
...
Matrix BundleAdjustment::getResiduo(Point* photogrammetricPoint) {
    Matrix result(1, 3);
    const int posFoto = whereInPhotogrammetricPoints(photogrammetricPoint);
...
Matrix BundleAdjustment::convertToUTM(Matrix coordinates, GeoSystem sys) {
    const int rows = coordinates.getRows();
...
double BundleAdjustment::calculateRMSE() {
    const int numRes = matRes.getRows();
    double sum = 0;
