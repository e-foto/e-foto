...
ReportManager::ReportManager():ReportManager(nullptr)
{
}

ReportManager::ReportManager(EFotoManager* manager):
    efotoManager_{manager},
    myInterface_{nullptr},
    started_{false}
{
    project_ = (manager)?manager->getProject():nullptr;
}
...
{
    if (efotoManager_ != NULL) {
        if (efotoManager_->getInterfaceType().compare("Qt") == 0) {
            myInterface_ = ReportUserInterface_Qt::instance(this);
        }
...
void ReportManager::checkTree(QList<QTreeWidgetItem*> treeItems)
{
    ...
    if(project_->allEOs().size() < 1 && project_->allIOs().size() < 1) {
        //images/orientation parameters
        treeItems.at(7)->setCheckState(0, Qt::Unchecked);
        treeItems.at(7)->setDisabled(true);
    }

    for(unsigned int i = 0; i < project_->allEOs().size(); i++) {
        SpatialRessection* sr = dynamic_cast<SpatialRessection*>(project_->allEOs().at(i));
...
{
    ProjectHeader* cabecalho = project_->header();
    std::string txt;
    txt = "<projectHeader>\n";
...
{
    Terrain* terreno = project_->terrain();
    std::string txt;
    Dms lat(terreno->getCentralCoordLat());
    Dms lon(terreno->getCentralCoordLong());
    txt = "<terrain>\n";
    txt += "<meanAltitude>" + Conversion::doubleToString(terreno->getMeanAltitude(),
            3) + "</meanAltitude>\n";
    txt += "<minAltitude>" + Conversion::doubleToString(terreno->getMinAltitude(),
            3) + "</minAltitude>\n";
    txt += "<maxAltitude>" + Conversion::doubleToString(terreno->getMaxAltitude(),
            3) + "</maxAltitude>\n";
    ...
    txt += "<seconds>" + Conversion::doubleToString(lat.getSeconds(),
            2) + "</seconds>\n";
    ...
    txt += "<seconds>" + Conversion::doubleToString(lon.getSeconds(),
            2) + "</seconds>\n";
...
{
    Sensor* sensores = project_->sensor(1);
    std::string txt;
    txt = "<sensors>\n";
    txt += "<sensorId>" + sensores->getSensorId() + "</sensorId>\n";
    txt += "<description>" + sensores->getDescription() + "</description>\n";
    txt += "<focalDistance>\n";
    txt += "<value>" + Conversion::doubleToString(sensores->getFocalDistance(),
            3) + "</value>\n";
    txt += "<sigma>" + Conversion::doubleToString(sensores->getFocalDistanceSigma(),
            3) + "</sigma>\n";
    txt += "</focalDistance>\n";
    txt += "</sensors>\n";
    return txt;
...
std::string ReportManager::eprFlights()
{
    ...
    txt += "<mml:mfrac>\n";
    txt += "<mml:mn>1</mml:mn>\n";
    txt += "<mml:mn>" + Conversion::doubleToString(voos->getScaleDen(),
            0) + "</mml:mn>\n";
    txt += "</mml:mfrac>\n";
    txt += "</nominalScale>\n";
    txt += "<overlap>\n";
    txt += "<longitudinal>" + Conversion::doubleToString(
               voos->getLongitudinalOverlap(), 0) + "</longitudinal>\n";
    txt += "<transversal>" + Conversion::doubleToString(
               voos->getTransversalOverlap(), 0) + "</transversal>\n";
    txt += "</overlap>\n";
    txt += "</flights>\n";
    return txt;
}
...
std::string ReportManager::eprImages(QList<QTreeWidgetItem*> treeItems)
{
    ...
    for(unsigned int i = 0; i < project_->allImages().size(); i++) {
        txt += "<image>\n";
        img = project_->allImages().at(i);
        ...
        if(treeItems.at(7)->checkState(0) == 2) {
            ...
            if(project_->allEOs().size() > 0) {
                ...
                if (NULL != eo) {
                    ...
                    txt +=  "<phi>" + Conversion::doubleToString(eo->getXa().get(4,
                            1)) + "</phi>\n";
                    txt +=  "<omega>" + Conversion::doubleToString(eo->getXa().get(5,
                            1)) + "</omega>\n";
                    txt +=  "<kappa>" + Conversion::doubleToString(eo->getXa().get(6,
                            1)) + "</kappa>\n";
                    txt +=  "</Xa>\n";
                    txt += "</exteriorOrientation>\n";
                }
...
std::string ReportManager::eprBlockPoints(QList<QTreeWidgetItem*> treeItems)
{
    ...
    for(unsigned int i = 0; i < project_->allPoints().size(); i++) {
        Point* pnt = project_->allPoints().at(i);
        txt += "<point>\n";
        ...
        if(treeItems.at(9)->checkState(0) == 2) {
            ...
            txt += "<x>" + Conversion::doubleToString(pnt->getObjectCoordinate().getX()) +
                   "</x>\n";
            txt += "<y>" + Conversion::doubleToString(pnt->getObjectCoordinate().getY()) +
                   "</y>\n";
            txt += "<z>" + Conversion::doubleToString(pnt->getObjectCoordinate().getZ()) +
                   "</z>\n";
            txt += "<unit>" + pnt->getObjectCoordinate().getUnit().substr(1,
                    pnt->getObjectCoordinate().getUnit().size()) + "</unit>\n";
            txt += "</spatialCoordinates>\n";
        }
        ...
        if(treeItems.at(10)->checkState(0) == 2) {
            ...
            for(unsigned int j = 0; j < pnt->getImageCoordinates().size(); j++) {
                Image* img = project_->image(pnt->getImageCoordinateAt(j).getImageId());

                if (img) {
                    txt += "<imageCoordinates>\n";
                    txt += "<imageId>" + img->getImageId() + "</imageId>\n";
                    txt += "<position>\n";
                    txt += "<lin>" + Conversion::doubleToString(pnt->getImageCoordinateAt(
                                j).getLin()) + "</lin>\n";
                    txt += "<col>" + Conversion::doubleToString(pnt->getImageCoordinateAt(
                                j).getCol()) + "</col>\n";
                    txt += "<unit>" + pnt->getImageCoordinateAt(j).getUnit().substr(1,
                            pnt->getImageCoordinateAt(j).getUnit().size()) + "</unit>\n";
                    txt += "</position>\n";
                    txt += "</imageCoordinates>\n";
                }
            }
...
std::string ReportManager::eprAffineTransformation(QList<QTreeWidgetItem*>
        treeItems)
{
    std::string txt;
    txt = "<affineTransformation>\n";

    for (unsigned int i = 0; i < project_->allIOs().size(); i++) {
        InteriorOrientation* io = project_->allIOs().at(i);
        ...
        if( NULL != io) {
            if(NULL != io->getImage()) {
                ...
                if(treeItems.at(12)->checkState(0) == 2) {
                    ...
                    txt += "<calculationMode>" + io->getImage()->getSensor()->getCalculationMode() +
                           "</calculationMode>\n";
                    ...
                    if (io->getImage()->getSensor()->getCalculationMode() ==
                        ...
                        for(unsigned int j = 0; j < fidMarks.size(); j++) {
                            txt += "<fidMark>\n";
                            QStringList gmlpos = QString::fromLocal8Bit(sensor->getAnaFidMarkAt(
                                                     j).getGmlPos().c_str()).split(" ");
                            txt += "<fidMarkNo>" + Conversion::intToString(j + 1) + "</fidMarkNo>\n";
                            txt += "<gmlX>" + std::string(gmlpos.at(0).toLocal8Bit().constData()) +
                                   "</gmlX>\n";
                            txt += "<gmlY>" + std::string(gmlpos.at(1).toLocal8Bit().constData()) +
                                   "</gmlY>\n";
                            txt += "<lin>" + Conversion::doubleToString(io->getImage()->getDigFidMarkAt(
                                        j).getLin()) + "</lin>\n";
                            txt += "<col>" + Conversion::doubleToString(io->getImage()->getDigFidMarkAt(
                                        j).getCol()) + "</col>\n";
                            txt += "</fidMark>\n";
                        }
                        ...
                        SensorWithKnowDimensions* sensor = dynamic_cast<SensorWithKnowDimensions*>
                                                           (io->getImage()->getSensor());
                        txt += "<sensorCols>" + Conversion::intToString(sensor->getFrameColumns()) +
                               "</sensorCols>\n";
                        txt += "<sensorRows>" + Conversion::intToString(sensor->getFrameRows()) +
                               "</sensorRows>\n";
                        txt += "<sensorPxsize>" + Conversion::doubleToString(sensor->getPixelSize()) +
                               "</sensorPxsize>\n";
                    } else if (io->getImage()->getSensor()->getCalculationMode() ==
                               "Fixed Parameters") {
                        SensorWithKnowParameters* sensor = dynamic_cast<SensorWithKnowParameters*>
                                                           (io->getImage()->getSensor());
                        txt += "<Xa>\n";
                        txt += "<a0>" + Conversion::doubleToString(sensor->getXa().get(1,
                                1)) + "</a0>\n";
                        txt += "<a1>" + Conversion::doubleToString(sensor->getXa().get(2,
                                1)) + "</a1>\n";
                        txt += "<a2>" + Conversion::doubleToString(sensor->getXa().get(3,
                                1)) + "</a2>\n";
                        txt += "<b0>" + Conversion::doubleToString(sensor->getXa().get(4,
                                1)) + "</b0>\n";
                        txt += "<b1>" + Conversion::doubleToString(sensor->getXa().get(5,
                                1)) + "</b1>\n";
                        txt += "<b2>" + Conversion::doubleToString(sensor->getXa().get(6,
                                1)) + "</b2>\n";
                        txt += "</Xa>\n";
                    }
                ...
                if(treeItems.at(13)->checkState(0) == 2) {
                    ...
                    txt += "</sigmaLa>\n";
                    txt += "<sigma0Squared>" + Conversion::doubleToString(
                               io->getQuality().getsigma0Squared()) + "</sigma0Squared>\n";
                    txt += "</qualityData>\n";
                }
...
std::string ReportManager::eprSpatialRessection(QList<QTreeWidgetItem*>
        treeItems)
{
    std::string txt;
    ...
    for(unsigned int i = 0; i < project_->allEOs().size(); i++) {
        ...
        txt += "<flightDirection>" + Conversion::doubleToString(
                   sr->getImage()->getFlightDirection()) + "</flightDirection>\n";
        txt += "<unit>rad</unit>\n";
        ...
        if(treeItems.at(15)->checkState(0) == 2) {
            ...
            if(sr->getImage()->isGnssAvailable()) {
                ...
                txt += "<x0>" + Conversion::doubleToString(sr->getImage()->getGnssX0()) +
                       "</x0>";
                txt += "<y0>" + Conversion::doubleToString(sr->getImage()->getGnssY0()) +
                       "</y0>";
                txt += "<z0>" + Conversion::doubleToString(sr->getImage()->getGnssZ0()) +
                       "</z0>";
                txt += "<gnss>\n";
            }
            ...
            if(sr->getImage()->isInsAvailable()) {
                ...
                txt += "<omega>" + Conversion::doubleToString(sr->getImage()->getInsOmega()) +
                       "</omega>\n";
                txt += "<phi>" + Conversion::doubleToString(sr->getImage()->getInsPhi()) +
                       "</phi>\n";
                txt += "<kappa>" + Conversion::doubleToString(sr->getImage()->getInsKappa()) +
                       "</kappa>\n";
                txt += "</ins>\n";
            }
            ...
            if(sr->getSelectedPoints().size() > 0) {
                ...
                for(unsigned int j = 0; j < sr->getSelectedPoints().size(); j++) {
                    Point* point = project_->point(sr->getSelectedPoints().at(j));
                    txt += "<point>\n";
                    txt += "<x>" + Conversion::doubleToString(point->getObjectCoordinate().getX()) +
                           "</x>\n";
                    txt += "<y>" + Conversion::doubleToString(point->getObjectCoordinate().getY()) +
                           "</y>\n";
                    txt += "<z>" + Conversion::doubleToString(point->getObjectCoordinate().getZ()) +
                           "</z>\n";
                    txt += "<col>" + Conversion::doubleToString(point->getImageCoordinate(
                                sr->getImageId()).getCol()) + "</col>\n";
                    txt += "<lin>" + Conversion::doubleToString(point->getImageCoordinate(
                                sr->getImageId()).getLin()) + "</lin>\n";
                    txt += "</point>\n";
                }
        ...
        if(treeItems.at(16)->checkState(0) == 2) {
            ...
            txt += "</sigmaLa>\n";
            txt += "<sigma0Squared>" + Conversion::doubleToString(
                       sr->getQuality().getsigma0Squared()) + "</sigma0Squared>\n";
            txt += "</qualityData>\n";
        }
...
std::string ReportManager::eprPhotogrammetricBlock(QList<QTreeWidgetItem*>
        treeItems)
{
    std::string txt;

    if (project_->photoTri() == NULL) {
        return "";
    }

    std::deque<Image*> usedImages = project_->photoTri()->getImages();
    std::deque<Point*> usedPoints = project_->photoTri()->getPoints();
    ...
    if(treeItems.at(19)->checkState(0) == 2) {
        txt += "<totalIterations>" + Conversion::doubleToString(
                   project_->photoTri()->getTotalIterations()) + "</totalIterations>\n";
        txt += "<angularConvergency>" + Conversion::doubleToString(
                   project_->photoTri()->getAngularConvergency()) + "</angularConvergency>\n";
        txt += "<metricConvergency>" + Conversion::doubleToString(
                   project_->photoTri()->getMetricConvergency()) + "</metricConvergency>\n";
        ...
        for(unsigned int i = 1; i < inicialValues.getRows() + 1; i++) {
            ...
            txt += "<imageId>" + img->getImageId() + "</imageId>\n";
            txt += "<matrix>\n";
            txt += "<omega>" + Conversion::doubleToString(inicialValues.get(i,
                    1)) + "</omega>\n";
            txt += "<phi>" + Conversion::doubleToString(inicialValues.get(i,
                    2)) + "</phi>\n";
            txt += "<kappa>" + Conversion::doubleToString(inicialValues.get(i,
                    3)) + "</kappa>\n";
    ...
    if(treeItems.at(20)->checkState(0) == 2) {
        bundle->setAngularConvergencyValue(
            project_->photoTri()->getAngularConvergency());
        bundle->setMetricConvergencyValue(project_->photoTri()->getMetricConvergency());
        bundle->setMaxNumberIterations(project_->photoTri()->getTotalIterations());
...
std::string ReportManager::eprInteriorOrientation(QList<QTreeWidgetItem*>
        treeItems)
{
    std::string txt;
    txt = "<interiorOrientation>\n";
...
std::string ReportManager::eprExteriorOrientation(QList<QTreeWidgetItem*>
        treeItems)
{
    std::string txt;
    Image* img;
    ...
    for(unsigned int i = 0; i < project_->allImages().size(); i++) {
        ...
        if(NULL != eo) {
            ...
            if(treeItems.at(27)->checkState(0) == 2) {
                ...
                if (eo->getType() == "photoTriangulation" && project_->photoTri()) {
                    txt += Conversion::intToString(project_->photoTri()->getTotalIterations());
...
std::string ReportManager::eprStereoPairs()
{
    ...
    for (unsigned int i = 0; i < project_->allImages().size() - 1; i++) {
        ...
        for (unsigned int j = i + 1; j < project_->allImages().size(); j++) {
            img = project_->allImages().at(j);
            Xa = img->getEO()->getXa();
            X2 = Xa.get(1, 1);
            Y2 = Xa.get(2, 1);
            // Calculate dist
        ...
        txt += "<stereoPair>\n";
        txt += "<imageId>" + project_->allImages().at(i)->getImageId() + "</imageId>\n";
        txt += "<imageId>" + project_->allImages().at(best_img)->getImageId() +
               "</imageId>\n";
        txt += "</stereoPair>\n";
    }
