...
OrthoManager::OrthoManager():OrthoManager(nullptr,
                                          std::deque<Image*>(),
...
bool OrthoManager::exec()
{
    ...
    if (listAllImages.size() < 1) {
        OrthoUserInterface_Qt* oui = dynamic_cast<OrthoUserInterface_Qt*>(myInterface);
...
void OrthoManager::runAllOrthoTogether()
{
    ...
    for (size_t curr_image = 0; curr_image < listAllImages.size(); curr_image++) {
        if (flag_cancel) {
            return;
        }
        ...
        for (double Y = Yi; Y < Yf; Y += res_y) {
            for (double X = Xi; X < Xf; X += res_x) {
                ...
                for (size_t k = 0; k < listAllImages.size(); k++) {
                ...
                if (best_img == static_cast<int>(curr_image)) {
                    (best_img == -1) ? pixel = 0.0 : pixel = interp.interpolate(&img_m, best_col,
...
void OrthoManager::setProgress(int progress) const
{
    OrthoUserInterface_Qt* oui = dynamic_cast<OrthoUserInterface_Qt*>(myInterface);
