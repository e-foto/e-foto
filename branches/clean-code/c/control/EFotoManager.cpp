 ...
 *
 */
Sensor* EFotoManager::instanceSensor(const int id)
{
    ...
    EDomElement root(xmlData);
    EDomElement xmlSensor = root.elementByTagAtt("sensor", "key",
    ...
    if (xmlSensor.elementByTagName("geometry").toString().compare("frame") == 0 &&
            xmlSensor.elementByTagName("calculationMode").toString().compare("With Fiducial Marks")
            == 0) {
        SensorWithFiducialMarks* newSensorWithFiducialMarks = new
    ...
    if (xmlSensor.elementByTagName("geometry").toString().compare("frame") == 0 &&
            xmlSensor.elementByTagName("calculationMode").toString().compare("With Sensor Dimensions")
            == 0) {
        SensorWithKnowDimensions* newSensorWithKnowDimensions = new
    ...
    if (xmlSensor.elementByTagName("geometry").toString().compare("frame") == 0 &&
            xmlSensor.elementByTagName("calculationMode").toString().compare("Fixed Parameters")
            == 0) {
        SensorWithKnowParameters* newSensorWithKnowParameters = new
...
Flight* EFotoManager::instanceFlight(int id)
{
    ...
    EDomElement root(xmlData);
    EDomElement xmlFlight = root.elementByTagAtt("flight", "key",
                            Conversion::intToString(id));
...
Image* EFotoManager::instanceImage(int id)
{
    ...
    EDomElement root(xmlData);
    EDomElement xmlImage = root.elementByTagAtt("image", "key",
                           Conversion::intToString(id));
...
void EFotoManager::instanceAllImages()
{
    ...
    for (unsigned int i = 0 ; i < xmlAllImages.size(); i++) {
        ...
        for (size_t j = 0; j < images.size() && notAvailable; j++)
            if (images.at(j)->getId() == Conversion::stringToInt(
                        xmlAllImages.at(i).attribute("key"))) {
                notAvailable = false;
            }
...
Point* EFotoManager::instancePoint(int id)
{
    ...
    EDomElement root(xmlData);
    EDomElement xmlPoint = root.elementByTagAtt("point", "key",
                           Conversion::intToString(id));
...
{
    unsigned int i;
    ExteriorOrientation* myEO = nullptr;
...
std::string EFotoManager::xmlGetData() const
{
    return xmlData;
}
...
bool EFotoManager::getSavedState() const
{
 ...
 *
 */
std::string EFotoManager::getInterfaceType() const
{
...
bool EFotoManager::execProject(std::string filename)
{
    ...
    if (project_ == nullptr) {
        project_ = new ProjectManager(this);
    }

    result = project_->exec(filename);
    return result;
}
...
bool EFotoManager::execSR(int id)
{
    ...
    SpatialRessection* sr = dynamic_cast<SpatialRessection*>(instanceEO(id));
    ...
    instanceAllPoints();
    spatialRessection = new SRManager(this, srTerrain, srSensor, srFlight, srImage,
                                      srIO, sr, points);
...
bool EFotoManager::existPair(int& id1, int& id2,
                             std::deque<int>& listPairs) const
{
...
int EFotoManager::getPairs(std::deque<int>& listPairs)
{
    ...
    img = images.at(0);
    // New D calcul
    int maxDim = img->getWidth() > img->getHeight() ? img->getWidth() :
                 img->getHeight();
    ...
    for (size_t i = 0; i < imagesSize; i++) {
        ...
        for (size_t j = i + 1; j < imagesSize; j++) {
            ...
            double avg = (flight->getLongitudinalOverlap() +
                          flight->getTransversalOverlap()) / 2.0;
