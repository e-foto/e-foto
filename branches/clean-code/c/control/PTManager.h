...
public:
    ...
    explicit PTManager(EFotoManager* newManager, std::deque<Image*> images,
                       std::deque<InteriorOrientation*> ois, Sensor* sensor, Flight* flight, Terrain*);
...
// Matrix getFotoIndice(std::deque<Matrix*> imgs, std::deque<Matrix> IOs, std::deque<Matrix> EOs, int width, int height, Matrix& dim); //(GraphicWorkAround)
    ...
    */
    static std::string pointToKml(Point* pnt, int zona, int hemiLatitude, GeoSystem sys ,
    ...
    */
    void convertToNunes(std::deque<Point*> points, GeoSystem sys, int hemi,
                        int zona);
