...
//Paulo
void GeometryResource::removePoint(QString pointId)
{
	int numPoints=geometries_.size();
	for (int i=0;i<numPoints;i++)
	{
		int numGeo=geometries_.at(i).listPoints().size();
		for (int j=0;j<numGeo;j++)
		{
			Coord temp=geometries_.at(i).listPoints().at(j);
			if (temp.getLabel()==pointId)
			{
