...
public:
    explicit SingleTool(SingleDisplay* display) {_display = display; QTimer *timer = new QTimer(this); _scale = -1; _scaleSpin = nullptr; _posLabel = nullptr; _propagateMoveTo = _propagateScaleTo = nullptr;
...
public slots:
    void autoMove();
    void propagateMoveTo(SingleTool* tool = nullptr) {_propagateMoveTo = tool;}
    void propagateScaleTo(SingleTool* tool = nullptr) {_propagateScaleTo = tool;}
...
public:
    ...
    /* Methods into disuse:
     *
    void editMark(int key, QPointF location, Marker* marker = nullptr);
    void editMark(int key, QPointF location, QString label, Marker* marker = nullptr);
