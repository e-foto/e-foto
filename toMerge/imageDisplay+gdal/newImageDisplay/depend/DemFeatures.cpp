...
FeatureClass * DemFeatures::getFeatureClass(unsigned classid)
{
...
int DemFeatures::exportTxtFeatures(char *filename)
{
    ...
    for (unsigned int i=0; i<features.size(); i++)
    {
        ...
        arq << " Class: " << getFeatureClass(unsigned(df.feature_class))->name.c_str() << "\n";
...
{
    //Hildermes, new fc inicializei com zero
    int new_fc = 0;
...
void DemFeatures::createClassesFromSp165()
{
    ...
    fc.fill_color = 0;
    fc.name = "Undefined";
    feature_classes.push_back(fc);//1

    // Point
    fc.type = 1;
    fc.outline_color = 0xFF0000;
    fc.name = "Point";
    feature_classes.push_back(fc);//2
    ...
    feature_classes.push_back(fc);//3
