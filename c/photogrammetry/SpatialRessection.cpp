...
bool SpatialRessection::is(const std::string& s) const
{
...
{
    std::stringstream result;
    result << "<imageSR image_key=\"" << Conversion::intToString(
               imageId) << "\">\n";
    ...
    for (int i = 0; i < (int)selectedPoints.size(); i++) {
        result << "<pointKey>" <<	Conversion::intToString(selectedPoints.at(
                    i))	<< "</pointKey>\n";
    }
    ...
    result << "</L0>\n";
    result << "<X0>\n";
    result << "<X00 uom=\"#m\">" << Conversion::doubleToString(X0.get(1,
            1)) << "</X00>\n";
    result << "<Y00 uom=\"#m\">" << Conversion::doubleToString(X0.get(2,
            1)) << "</Y00>\n";
    result << "<Z00 uom=\"#m\">" << Conversion::doubleToString(X0.get(3,
            1)) << "</Z00>\n";
    result << "<phi0 uom=\"#rad\">" << Conversion::doubleToString(X0.get(4,
            1)) << "</phi0>\n";
    result << "<omega0 uom=\"#rad\">" << Conversion::doubleToString(X0.get(5,
            1)) << "</omega0>\n";
    result << "<kappa0 uom=\"#rad\">" << Conversion::doubleToString(X0.get(6,
            1)) << "</kappa0>\n";
    result << "</X0>\n";
    result << "</parameters>\n";
...
std::string SpatialRessection::xmlGetDataEO()
{
    ...
    result << "<Xa>\n";
    result << "<X0 uom=\"#m\">" << Conversion::doubleToString(Xa.get(1,
            1)) << "</X0>\n";
    result << "<Y0 uom=\"#m\">" << Conversion::doubleToString(Xa.get(2,
            1)) << "</Y0>\n";
    result << "<Z0 uom=\"#m\">" << Conversion::doubleToString(Xa.get(3,
            1)) << "</Z0>\n";
    result << "<phi uom=\"#rad\">" << Conversion::doubleToString(Xa.get(4,
            1)) << "</phi>\n";
    result << "<omega uom=\"#rad\">" << Conversion::doubleToString(Xa.get(5,
            1)) << "</omega>\n";
    result << "<kappa uom=\"#rad\">" << Conversion::doubleToString(Xa.get(6,
            1)) << "</kappa>\n";
    result << "</Xa>\n";
    result << "</imageEO>\n";
    return result.str();
...
void SpatialRessection::generateInitialA()
{
    if (myImage != NULL) {
        ...
        for (unsigned int i = 0, j = 1; i < selectedPoints.size(); i++, j += 2) {
            ...
            Point* myPoint = myImage->getPoint(selectedPoints.at(
                                                   i)); // Escolhi aqui o ponto que estou usando.
            DetectorSpaceCoordinate myCoordinate = myPoint->getDetectorCoordinate(
                    myImage->getId()); // Tento tirar as coordenadas analógicas dele.

            if (myCoordinate.getUnit() ==
                    "") { // Se essas coordenadas analógicas não existirem, acusadas pela falta de unidade...
                myCoordinate = rt->imageToDetector(myPoint->getImageCoordinate(
                                                       myImage->getId()));
            }
...
void SpatialRessection::generateA()
{
    if (myImage != NULL) {
        ...
        for (unsigned int i = 0, j = 1; i < selectedPoints.size(); i++, j += 2) {
            ...
            A.set(j, 1, c * cos(kappa0)*cos(phi0) / (sin(phi0) * (X - X00) - cos(phi0)*sin(
                        omega0) * (Y - Y00) + cos(phi0)*cos(omega0) * (Z - Z00)) - c * (cos(kappa0)*cos(
                                    phi0) * (X - X00) + (sin(kappa0)*cos(omega0) + cos(kappa0)*sin(phi0)*sin(
                                                omega0)) * (Y - Y00) + (sin(kappa0)*sin(omega0) - cos(kappa0)*sin(phi0)*cos(
                                                        omega0)) * (Z - Z00)) / pow((sin(phi0) * (X - X00) - cos(phi0)*sin(omega0) *
                                                                (Y - Y00) + cos(phi0)*cos(omega0) * (Z - Z00)), 2)*sin(phi0));
            A.set(j, 2, -c * (-sin(kappa0)*cos(omega0) - cos(kappa0)*sin(phi0)*sin(
                                  omega0)) / (sin(phi0) * (X - X00) - cos(phi0)*sin(omega0) * (Y - Y00) + cos(
                                                  phi0)*cos(omega0) * (Z - Z00)) + c * (cos(kappa0)*cos(phi0) * (X - X00) + (sin(
                                                          kappa0)*cos(omega0) + cos(kappa0)*sin(phi0)*sin(omega0)) * (Y - Y00) + (sin(
                                                                  kappa0)*sin(omega0) - cos(kappa0)*sin(phi0)*cos(omega0)) * (Z - Z00)) / pow((
                                                                          sin(phi0) * (X - X00) - cos(phi0)*sin(omega0) * (Y - Y00) + cos(phi0)*cos(
                                                                                  omega0) * (Z - Z00)), 2)*cos(phi0)*sin(omega0));
            A.set(j, 3, -c * (-sin(kappa0)*sin(omega0) + cos(kappa0)*sin(phi0)*cos(
                                  omega0)) / (sin(phi0) * (X - X00) - cos(phi0)*sin(omega0) * (Y - Y00) + cos(
                                                  phi0)*cos(omega0) * (Z - Z00)) - c * (cos(kappa0)*cos(phi0) * (X - X00) + (sin(
                                                          kappa0)*cos(omega0) + cos(kappa0)*sin(phi0)*sin(omega0)) * (Y - Y00) + (sin(
                                                                  kappa0)*sin(omega0) - cos(kappa0)*sin(phi0)*cos(omega0)) * (Z - Z00)) / pow((
                                                                          sin(phi0) * (X - X00) - cos(phi0)*sin(omega0) * (Y - Y00) + cos(phi0)*cos(
                                                                                  omega0) * (Z - Z00)), 2)*cos(phi0)*cos(omega0));
            A.set(j, 4, -c * (-cos(kappa0)*sin(phi0) * (X - X00) + cos(kappa0)*cos(
                                  phi0)*sin(omega0) * (Y - Y00) - cos(kappa0)*cos(phi0)*cos(omega0) *
                              (Z - Z00)) / (sin(phi0) * (X - X00) - cos(phi0)*sin(omega0) * (Y - Y00) + cos(
                                                phi0)*cos(omega0) * (Z - Z00)) + c * (cos(kappa0)*cos(phi0) * (X - X00) + (sin(
                                                        kappa0)*cos(omega0) + cos(kappa0)*sin(phi0)*sin(omega0)) * (Y - Y00) + (sin(
                                                                kappa0)*sin(omega0) - cos(kappa0)*sin(phi0)*cos(omega0)) * (Z - Z00)) / pow((
                                                                        sin(phi0) * (X - X00) - cos(phi0)*sin(omega0) * (Y - Y00) + cos(phi0)*cos(
                                                                                omega0) * (Z - Z00)), 2) * (cos(phi0) * (X - X00) + sin(phi0)*sin(omega0) *
                                                                                        (Y - Y00) - sin(phi0)*cos(omega0) * (Z - Z00)));
            A.set(j, 5, -c * ((-sin(kappa0)*sin(omega0) + cos(kappa0)*sin(phi0)*cos(
                                   omega0)) * (Y - Y00) + (sin(kappa0)*cos(omega0) + cos(kappa0)*sin(phi0)*sin(
                                               omega0)) * (Z - Z00)) / (sin(phi0) * (X - X00) - cos(phi0)*sin(omega0) *
                                                       (Y - Y00) + cos(phi0)*cos(omega0) * (Z - Z00)) + c * (cos(kappa0)*cos(phi0) *
                                                               (X - X00) + (sin(kappa0)*cos(omega0) + cos(kappa0)*sin(phi0)*sin(omega0)) *
                                                               (Y - Y00) + (sin(kappa0)*sin(omega0) - cos(kappa0)*sin(phi0)*cos(omega0)) *
                                                               (Z - Z00)) / pow((sin(phi0) * (X - X00) - cos(phi0)*sin(omega0) *
                                                                       (Y - Y00) + cos(phi0)*cos(omega0) * (Z - Z00)),
                                                                       2) * (-cos(phi0)*cos(omega0) * (Y - Y00) - cos(phi0)*sin(omega0) * (Z - Z00)));
            A.set(j, 6, -c * (-sin(kappa0)*cos(phi0) * (X - X00) + (cos(kappa0)*cos(
                                  omega0) - sin(kappa0)*sin(phi0)*sin(omega0)) * (Y - Y00) + (cos(kappa0)*sin(
                                              omega0) + sin(kappa0)*sin(phi0)*cos(omega0)) * (Z - Z00)) / (sin(phi0) *
                                                      (X - X00) - cos(phi0)*sin(omega0) * (Y - Y00) + cos(phi0)*cos(omega0) *
                                                      (Z - Z00)));
            A.set(j + 1, 1, -c * sin(kappa0)*cos(phi0) / (sin(phi0) * (X - X00) - cos(
                        phi0)*sin(omega0) * (Y - Y00) + cos(phi0)*cos(omega0) * (Z - Z00)) - c * (-sin(
                                    kappa0)*cos(phi0) * (X - X00) + (cos(kappa0)*cos(omega0) - sin(kappa0)*sin(
                                                phi0)*sin(omega0)) * (Y - Y00) + (cos(kappa0)*sin(omega0) + sin(kappa0)*sin(
                                                        phi0)*cos(omega0)) * (Z - Z00)) / pow((sin(phi0) * (X - X00) - cos(phi0)*sin(
                                                                omega0) * (Y - Y00) + cos(phi0)*cos(omega0) * (Z - Z00)), 2)*sin(phi0));
            A.set(j + 1, 2, -c * (-cos(kappa0)*cos(omega0) + sin(kappa0)*sin(phi0)*sin(
                                      omega0)) / (sin(phi0) * (X - X00) - cos(phi0)*sin(omega0) * (Y - Y00) + cos(
                                              phi0)*cos(omega0) * (Z - Z00)) + c * (-sin(kappa0)*cos(phi0) * (X - X00) + (cos(
                                                      kappa0)*cos(omega0) - sin(kappa0)*sin(phi0)*sin(omega0)) * (Y - Y00) + (cos(
                                                              kappa0)*sin(omega0) + sin(kappa0)*sin(phi0)*cos(omega0)) * (Z - Z00)) / pow((
                                                                      sin(phi0) * (X - X00) - cos(phi0)*sin(omega0) * (Y - Y00) + cos(phi0)*cos(
                                                                              omega0) * (Z - Z00)), 2)*cos(phi0)*sin(omega0));
            A.set(j + 1, 3, -c * (-cos(kappa0)*sin(omega0) - sin(kappa0)*sin(phi0)*cos(
                                      omega0)) / (sin(phi0) * (X - X00) - cos(phi0)*sin(omega0) * (Y - Y00) + cos(
                                              phi0)*cos(omega0) * (Z - Z00)) - c * (-sin(kappa0)*cos(phi0) * (X - X00) + (cos(
                                                      kappa0)*cos(omega0) - sin(kappa0)*sin(phi0)*sin(omega0)) * (Y - Y00) + (cos(
                                                              kappa0)*sin(omega0) + sin(kappa0)*sin(phi0)*cos(omega0)) * (Z - Z00)) / pow((
                                                                      sin(phi0) * (X - X00) - cos(phi0)*sin(omega0) * (Y - Y00) + cos(phi0)*cos(
                                                                              omega0) * (Z - Z00)), 2)*cos(phi0)*cos(omega0));
            A.set(j + 1, 4, -c * (sin(kappa0)*sin(phi0) * (X - X00) - sin(kappa0)*cos(
                                      phi0)*sin(omega0) * (Y - Y00) + sin(kappa0)*cos(phi0)*cos(omega0) *
                                  (Z - Z00)) / (sin(phi0) * (X - X00) - cos(phi0)*sin(omega0) * (Y - Y00) + cos(
                                                    phi0)*cos(omega0) * (Z - Z00)) + c * (-sin(kappa0)*cos(phi0) * (X - X00) + (cos(
                                                            kappa0)*cos(omega0) - sin(kappa0)*sin(phi0)*sin(omega0)) * (Y - Y00) + (cos(
                                                                    kappa0)*sin(omega0) + sin(kappa0)*sin(phi0)*cos(omega0)) * (Z - Z00)) / pow((
                                                                            sin(phi0) * (X - X00) - cos(phi0)*sin(omega0) * (Y - Y00) + cos(phi0)*cos(
                                                                                    omega0) * (Z - Z00)), 2) * (cos(phi0) * (X - X00) + sin(phi0)*sin(omega0) *
                                                                                            (Y - Y00) - sin(phi0)*cos(omega0) * (Z - Z00)));
            A.set(j + 1, 5, -c * ((-cos(kappa0)*sin(omega0) - sin(kappa0)*sin(phi0)*cos(
                                       omega0)) * (Y - Y00) + (cos(kappa0)*cos(omega0) - sin(kappa0)*sin(phi0)*sin(
                                                   omega0)) * (Z - Z00)) / (sin(phi0) * (X - X00) - cos(phi0)*sin(omega0) *
                                                           (Y - Y00) + cos(phi0)*cos(omega0) * (Z - Z00)) + c * (-sin(kappa0)*cos(phi0) *
                                                                   (X - X00) + (cos(kappa0)*cos(omega0) - sin(kappa0)*sin(phi0)*sin(omega0)) *
                                                                   (Y - Y00) + (cos(kappa0)*sin(omega0) + sin(kappa0)*sin(phi0)*cos(omega0)) *
                                                                   (Z - Z00)) / pow((sin(phi0) * (X - X00) - cos(phi0)*sin(omega0) *
                                                                           (Y - Y00) + cos(phi0)*cos(omega0) * (Z - Z00)),
                                                                           2) * (-cos(phi0)*cos(omega0) * (Y - Y00) - cos(phi0)*sin(omega0) * (Z - Z00)));
            A.set(j + 1, 6, -c * (-cos(kappa0)*cos(phi0) * (X - X00) + (-sin(kappa0)*cos(
                                      omega0) - cos(kappa0)*sin(phi0)*sin(omega0)) * (Y - Y00) + (-sin(kappa0)*sin(
                                                  omega0) + cos(kappa0)*sin(phi0)*cos(omega0)) * (Z - Z00)) / (sin(phi0) *
                                                          (X - X00) - cos(phi0)*sin(omega0) * (Y - Y00) + cos(phi0)*cos(omega0) *
                                                          (Z - Z00)));
        }
    }
...
{
    if (myImage != NULL && myImage->getSensor() != NULL
            && myImage->getFlight() != NULL
            && myImage->getIO() !=
            NULL) {
        if (rt == NULL) {
            rt = new RayTester(myImage);
        ...
        Z00 = meanAltitude + myImage->getSensor()->getFocalDistance() * 0.5 * (sqrt(
                    Xa.get(2, 1) * Xa.get(2, 1) + Xa.get(3, 1) * Xa.get(3, 1)) + sqrt(Xa.get(5,
                            1) * Xa.get(5, 1) + Xa.get(6, 1) * Xa.get(6, 1)));
        ...
        if (!myImage->isGnssAvailable() || (myImage->isGnssAvailable()
                                            && myImage->getGnssType() == "Unused")) {
            X0.set(1, 1, X00);
            X0.set(2, 1, Y00);
            X0.set(3, 1, Z00);
        }
        ...
        if (!myImage->isInsAvailable() || (myImage->isInsAvailable()
                                           && myImage->getInsType() == "Unused")) {
            X0.set(4, 1, phi0);
            X0.set(5, 1, omega0);
            X0.set(6, 1, kappa0);
        }
...
DetectorSpaceCoordinate SpatialRessection::getDecenteredDistortions(double xi,
        double eta)
{
    DetectorSpaceCoordinate result;
    result.setXi(0.0);
...
DetectorSpaceCoordinate SpatialRessection::getAtmosphereDistortions(double xi,
        double eta)
{
    DetectorSpaceCoordinate result;
    result.setXi(0.0);
    ...
    if (flight != NULL && sensor != NULL) {
        ...
        if (terrain != NULL) {
            ...
            Z0 = myImage->getFlight()->getScaleDen() *
                 myImage->getSensor()->getFocalDistance() / 1000 + terrain->getMeanAltitude();
...
void SpatialRessection::setImageId(int newImageId)
{
    imageId = newImageId;
}
 ...
 */
void SpatialRessection::setImage(Image* newImage)
{
    myImage = newImage;
}

std::string SpatialRessection::getType()
{
    return type;
}
