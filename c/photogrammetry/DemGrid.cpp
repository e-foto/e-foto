...
double DemGrid::getHeight(double row, double col) const
{
    ...
    if (col >= double(dem_width)) {
        col = double(dem_width) - 1;
    }

    if (row >= double(dem_height)) {
        row = double(dem_height) - 1;
    }
...
void DemGrid::interpolateTrendSurfaceFast(int mode)
{
    ...
    switch (mode) {
    case 0 :
        A.resize(no_points, 3);
        break;

    case 1 :
        A.resize(no_points, 4);
        break;

    case 2 :
        A.resize(no_points, 5);
        break;

    case 3 :
        A.resize(no_points, 6);
        break;

    default :
        A.resize(no_points, 10);
        break;
    }
...
void DemGrid::interpolateMovingSurfaceFast(double n, double D0, int mode, int mode2)
{
    ...
    for (unsigned int y = 1; y <= DEM.getRows(); y++) {
        for (unsigned int x = 1; x <= DEM.getCols(); x++) {
            ...
            switch (mode2) {
            case 0 :
                A.resize(no_valid_points, 3);
                break;

            case 1 :
                A.resize(no_valid_points, 4);
                break;

            case 2 :
                A.resize(no_valid_points, 5);
                break;

            case 3 :
                A.resize(no_valid_points, 6);
                break;

            default :
                A.resize(no_valid_points, 10);
                break;
            }
            ...
            switch (mode2) {
            case 0 :
                DEM.set(y, x, X.get(1, 1) + X.get(2, 1)*Px + X.get(3, 1)*Py);
                break;

            case 1 :
                DEM.set(y, x, X.get(1, 1) + X.get(2, 1)*Px + X.get(3, 1)*Py + X.get(4, 1)*Px * Py);
                break;

            case 2 :
                DEM.set(y, x, X.get(1, 1) + X.get(2, 1)*Px + X.get(3, 1)*Py + X.get(4, 1)*Px * Px + X.get(5, 1)*Py * Py);
                break;

            case 3 :
                DEM.set(y, x, X.get(1, 1) + X.get(2, 1)*Px + X.get(3, 1)*Py + X.get(4, 1)*Px * Py + X.get(5, 1)*Px * Px + X.get(6, 1)*Py * Py);
                break;

            case 4 :
                DEM.set(y, x, X.get(1, 1) + X.get(2, 1)*Px + X.get(3, 1)*Py + X.get(4, 1)*Px * Py + X.get(5, 1)*Px * Px + X.get(6, 1)*Py * Py + X.get(7, 1)*Px * Px * Px + X.get(8, 1)*Px * Px * Py + X.get(9, 1)*Px * Py * Py + X.get(10, 1)*Py * Py * Py);
                break;
            }
...
void DemGrid::interpolateMovingSurfaceNormal(double n, double D0, int mode, int mode2)
{
    ...
    for (unsigned int y = 1; y <= DEM.getRows(); y++) {
        for (unsigned int x = 1; x <= DEM.getCols(); x++) {
            ...
            switch (mode2) {
            case 0 :
                DEM.set(y, x, X.get(1, 1) + X.get(2, 1)*Px
                        + X.get(3, 1)*Py);
                break;
