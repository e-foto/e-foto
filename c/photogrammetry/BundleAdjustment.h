...
* \brief Class that calculates the parameters of the exterior orientations of all images in a block
*/
...
public:
    ...
    */
    ...
    /**
    * \brief Calculates the m1 matrix, necessary for the initial calculation of the exterior orientation parameters
    * \param    M1   : Matrix formed by the analog coordinates of all points
    ...
    */
    ...
    /**
    * \brief Method that returns the matrix with the first approximations of the exterior orientation parameters
    * \param    M12  : Matrix M12
    * \param    m1   : Matrix m1
    * \param    xypf : Matrix xypf, matrix with the first approximations of the coordinates of the photogrammetric points
    * \return Matrix : Matrix paf, matrix with the first approximations of the exterior orientation parameters
    */
    Matrix getPAf(Matrix M12,Matrix m1, Matrix xypf);

    /**
    * \brief Method that returns the matrix with the first approximations of the coordinates of the photogrammetric points
    * \param    M22  : Matrix M22
    * \param    m2   : Matrix m2
    * \param    M12  : Matrix M12
    * \param    paf  : Matrix paf which contains the initial values ​​of the exterior orientation parameters
    * \return Matrix : Matrix xypf, matrix with the first approximations of the coordinates of the photogrammetric points
    */
    Matrix getXYpf(Matrix M22, Matrix m2, Matrix M12, Matrix paf);

    /**
    * \deprecated Initial guess Z values ​​are currently calculated based on the Flight form
    */
    double getZpf();
    ...
    *
    * \param   Paf   : Matrix with all the parameters of the exterior orientation of all the images
    * \param imageId : image id in current calculation
    * \attention the imageId is NOT the key of the image in the ProjectManager, but the line in which its parameters are found in the adjustment matrix(matAdjust)
    * \return Matrix : Matrix with the parameters of the exterior orientation of an image
    */
    ...
    */
    ...
    /**
    * \brief Method that returns the final version of the adjustment matrix (matAdjust)
    *        of the exterior orientation parameters
    *
    * \return Matrix : Matrix AFP that contains all parameters of all images with omega, phi and kappa in degrees
    */
    Matrix getAFP();

    /**
    * \brief Method that returns a matrix with the analog coordinates of the points calculated by the collinearity equations
    * \return Matrix : Returns the L0 matrix
    */
    Matrix getL0();

    /**
    * \brief Method that returns a matrix with the analog coordinates of the points calculated by interior orientation
    * \return Matrix : Returns the Lb matrix
    */
    Matrix getLb();
...
protected:
    ...
    * \attention rotation matrix values are contained in attributes double : r11,r12,r13,r21,r22,r23,r31,r32 e r33
    */
    void setRot(double omega, double phi, double kappa);

    /**
    * \brief Method that returns the x and straight values of an X Y Z coordinate contained in an image through collinearity equations
    ...
    */
    ...
    /**
    * \brief Method that returns a Jacobian matrix of an X Y Z coordinate of a Control point
    * \param    X    : Coordinate value on axis X
    * \param    Y    : Coordinate value on axis Y
    * \param    Z    : Coordinate value on axis Z
    * \param imageId : image id in current calculation
    ...
    */
    ...
    /**
    * \brief Method that returns the Jacobian matrix of an X Y Z coordinate of a photogrammetric point
    * \param    X    : Coordinate value on axis X
    * \param    Y    : Coordinate value on axis Y
    * \param    Z    : Coordinate value on axis Z
    * \param imageId : image id in current calculation
    ...
    */
    ...
    /**
    * \brief Creates or changes the Matrix A1 attribute, which is the matrix formed by the Jacobian matrices of all points as if they were control
    */
    void createA1();

    /**
    * \brief Creates or changes the Matrix A2 attribute, which is the matrix formed by the Jacobian matrices of the photogrammetric points only
    */
    void createA2();

    /**
    * \brief Method that creates or changes a matrix with the analogical coordinates of the points calculated by interior orientation
    */
    void createLb();

    /**
    * \brief Method that creates or changes a matrix with the analog coordinates of the points calculated by the collinearity equations
    */
    void createL0();
    ...
    */
    ...
    /**
    * \brief Calculates matrix n1, necessary for calculating the adjustment of the exterior orientation parameters
    ...
    */
    ...
    /**
    * \brief Calculates matrix n2, necessary for calculating the adjustment of the exterior orientation parameters
    * \param    l    : Difference matrix between Lb and L0
    ...
    * \attention Currently the weight matrix is ALWAYS the identity matrix in the calculation of the parameters of the exterior orientation
    */
    void setPeso(Matrix p);

    /**
    * \brief Returns the current weight matrix
    * \attention Currently, the weight matrix is ALWAYS the identity matrix in the calculation of the exterior orientation parameters
    */
    Matrix getPeso();

    /**
    * \brief Calculates and alters the matrix x1 which is the matrix that contains the "deltas" to be summed in the parameters of the exterior orientation
    ...
    */
    ...
    /**
    * \brief Calculates and alters the matrix x2 which is the matrix that contains the "deltas" to be added in the coordinates of the photogrammetric points
    * \param    N12  : Matrix N12
    ...
    */
    ...
    /**
    * \brief Auxiliary method that returns the Omega angle adjustment value (delta)
    *        contained in the x1 matrix
    *
    * \param imageId : image id in current calculation
    ...
    * \return double : Delta value of the Omega angle contained in the previously calculated matrix x1
    */
    double getdOmegax1(int imageId);

    /**
    * \brief Auxiliary method that returns the adjustment value (delta) of the Phi angle contained in the matrix x1
    * \param imageId : image id in current calculation
    * \attention the imageId is NOT the key of the image in the ProjectManager, but the line in which its parameters are found in the adjustment matrix (matAdjust)
    * \return double : Delta value of the Phi angle contained in the previously calculated matrix x1
    */
    double getdPhix1(int imageId);

    /**
    * \brief Auxiliary method that returns the adjustment value (delta) of the Kappa angle contained in the matrix x1
    * \param imageId : image id in current calculation
    * \attention the imageId is NOT the key of the image in the ProjectManager, but the line in which its parameters are found in the adjustment matrix (matAdjust)
    * \return double : Delta value of the Kappa angle contained in the previously calculated matrix x1
    */
    double getdKappax1(int imageId);

    /**
    * \brief Auxiliary method that returns the adjustment value (delta) of coordinate X0 contained in matrix x1
    * \param imageId : image id in current calculation
    * \attention the imageId is NOT the key of the image in the ProjectManager, but the line in which its parameters are found in the adjustment matrix (matAdjust)
    * \return double : Delta value of the X0 coordinate contained in the previously calculated x1 matrix
    */
    double getdXx1(int imageId);

    /**
    * \brief Auxiliary method that returns the adjustment value (delta) of the Y0 coordinate contained in the x1 matrix
    * \param imageId : image id in current calculation
    * \attention the imageId is NOT the key of the image in the ProjectManager, but the line in which its parameters are found in the adjustment matrix (matAdjust)
    * \return double : Delta value of the Y0 coordinate contained in the previously calculated x1 matrix
    */
    double getdYx1(int imageId);

    /**
    * \brief Auxiliary method that returns the adjustment value (delta) of the Z0 coordinate contained in the x1 matrix
    * \param imageId : image id in current calculation
    * \attention the imageId is NOT the key of the image in the ProjectManager, but the line in which its parameters are found in the adjustment matrix (matAdjust)
    * \return double : Delta value of the Z0 coordinate contained in the previously calculated x1 matrix
    */
    ...
    * \param imageId : image id in current calculation
    * \attention the imageId is NOT the key of the image in the ProjectManager, but the line in which its parameters are found in the adjustment matrix (matAdjust)
    * \return double : Value of the X0 coordinate contained in the matAdjust Matrix
    */
    double getXAdjus(int imageId);

    /**
    * \brief Auxiliary method that returns the value of the Y0 coordinate in the adjustment matrix
    * \param imageId : image id in current calculation
    * \attention the imageId is NOT the key of the image in the ProjectManager, but the line in which its parameters are found in the adjustment matrix (matAdjust)
    ...
    */
    ...
    /**
    * \brief Auxiliary method that returns the Z0 coordinate value in the adjustment matrix
    * \param imageId : image id in current calculation
    * \attention the imageId is NOT the key of the image in the ProjectManager, but the line in which its parameters are found in the adjustment matrix (matAdjust)
    ...
    * \attention the fotogrPointId is NOT the key or pointId of the point in the ProjectManager, but the position in the list of points where it is located
    * \return double : Delta value of the X coordinate of the point contained in the previously calculated x2 matrix
    */
    double getdXx2(int fotogrPointId);

    /**
    * \brief Auxiliary method that returns the adjustment value (delta) of the Y coordinate of the photogrammetric point contained in the matrix x2
    * \param fotogrPointId : id in current calculation
    * \attention the fotogrPointId is NOT the key or pointId of the point in the ProjectManager, but the position in the list of points where it is located
    * \return double : Delta value of the Y coordinate of the point contained in the previously calculated x2 matrix
    */
    double getdYx2(int fotogrPointId);

    /**
    * \brief Auxiliary method that returns the adjustment value (delta) of the Z coordinate of the photogrammetric point contained in the x2 matrix
    * \param fotogrPointId : id in current calculation
    * \attention the fotogrPointId is NOT the key or pointId of the point in the ProjectManager, but the position in the list of points where it is located
    * \return double : Delta value of the Z coordinate of the point contained in the previously calculated x2 matrix
    */
    double getdZx2(int fotogrPointId);

    /**
    * \brief Method that updates the parameter values of the exterior orientations using the values contained in the x1 matrix
    */
    void updateMatAdjust();

    /**
    * \brief Method that returns a matrix with terrain coordinates by spatial intersection equations
...
public:
    ...
    /**
    * \brief Method that fills Point objects with analog coordinates based on interior orientation
    */
    ...
    */
    ...
    /**
    * \brief Method that calculates the initial parameters of exterior orientations
    */
    void calculateInicialsValues();

    /**
    * \brief Changes the matrix of the initial parameters of the exterior orientations to those provided by the user in the interface
    * \param initialValues : Matrix with the initial parameters, with one image per line in the sequence: Omega, Phi, Kappa, X0, Y0, Z0
    */
    ...
    * \return bool: Informs whether the calculation was successful or not
    */
    bool calculate(bool makeReport);

    /**
    * \brief Method that informs whether it is possible to calculate the adjustment or not, based on the number of unknowns and equations
    ...
    * \return Matrix : returns the formed matrix M1 necessary for the initial calculation of the parameters
    */
    Matrix createM1();

    /**
    * \brief Method that creates or alters the matrix M2
    * \return Matrix : returns the formed matrix M2 necessary for the initial calculation of the parameters
    */
    Matrix createM2();

    /**
    * \brief Auxiliary method that calculates the initial Z coordinate for each photogrammetric point
    * \return double : Initial value of the Z coordinate of the photogrammetric points
    */
    double getInicialZPhotogrammetricPoints();

    /**
    * \brief Method that updates the initial coordinates of all photogrammetric points based on the xypf matrix
    ...
    */
    ...
    //These methods refer to their order in the LIST OF INDEX POINTS OF EACH IMAGE and not in the xml list
    /**
    * \brief Auxiliary method that tests whether a given point contained in a given image is photogrammetric
    * \param imageIndex : Image position in the image list
    * \param pointIndex : Position of the point in the list of points in the image informed by the parameter imageIndex
    * \return bool : informs if the point is photogrammetric or not
    */
    bool isPhotogrammetricPoint(int imageIndex, int pointIndex);

    /**
    * \brief Auxiliary method that tests whether a given point contained in a given image is a control one
    * \param imageIndex : Image position in the image list
    * \param pointIndex : Position of the point in the list of points in the image informed by the parameter imageIndex
    ...
    */
    ...
    /**
    * \brief Auxiliary method that tests whether a given point contained in a given image is a 'checking'
    * \param imageIndex : Image position in the image list
    * \param pointIndex : Position of the point in the list of points in the image informed by the parameter imageIndex
    ...
    */
    ...
    /**
    * \brief Method that informs whether the calculation converged or not based on the number of iterations and on the convergence value stipulated by the user
    ...
    * \return int : Code for the calculation convergence analysis, if the calculation converged it returns 1, if it did not converge and there were no problems in the matrices it returns 0, if there is any NAN in the matrix it returns -1, if there is any INF value it returns -2
    */
    int testConverged();
    ...
    * \deprecated Currently this test is not done because there is no iteration over the weight matrix P
    */
    bool testResiduo();
    ...
    */
    ...
    /**
    * \brief Auxiliary method that informs the number of photogrammetric points contained in an image
    ...
    */
    ...
    /**
    * \brief Auxiliary method that informs the position of a certain point in the list of all points
    ...
    */
    ...
    /**
    * \brief Auxiliary method that informs the position of a certain point in the list of photogrammetric points
    * \param pnt : Point object which you want to know its position in the list
    * \return int : Position in the list, if the point is not in the list returns -1;
    */
    ...
    * \deprecated Unnecessary method considering that the initial coordinates of the photogrammetric points are irrelevant for the calculation
    */
    void zeroingCoordinatesPhotogrammetrics();

    /**
    * \brief Auxiliary method that returns the residual at the X coordinate of a given point in a given image
    ...
    */
    ...
    /**
    * \brief Auxiliary method that returns the residual at the Y coordinate of a given point in a given image
    * \param img : Image object in which the point is contained
    * \param pointId : Position of the point in the list of points belonging to this image
    ...
    */
    ...
    /**
    * \brief Method that changes the value of the convergence of the coordinates of the parameters of the exterior orientations
    * \param value : Value of the convergence of coordinates
    */
    void setMetricConvergencyValue(double value);

    /**
    * \brief Method that changes the value of the convergence of the angles of the parameters of the exterior orientations
    * \param value : Value of convergence of angles
    */
    void setAngularConvergencyValue(double value);

    /**
    * \brief Method that returns the value of the convergence of the coordinates of the parameters of the exterior orientations
    * \return double : Coordinate convergence value
    */
    double getMetricConvergencyValue();

    /**
    * \brief Method that returns the value of the convergence of the angles of the parameters of the exterior orientations
    ...
    */
    ...
    /**
    * \brief Method that returns the x1 matrix formatted to display angles in degrees and each exterior orientation on a line
    * \return Matrix : formatted matrix
    */
    Matrix getMVC();

    /**
    * \brief Method that calculates the residuals after the calculation converges and changes the matRes attribute that stores the residuals
    */
    void calculateResiduos();

    /**
    * \brief Method that calculates the matrix of weights after the calculation converges and changes the attribute P that stores the weights
    */
    void calculatePeso();

    /**
    *	\brief Method that updates, that is, adjusts the coordinates of photogrammetric points based on the x2 matrix
    */
    void updateCoordFotog();


    /**
    * \brief Method that inverts only the main diagonal matrices of the m11 matrix, thus avoiding unnecessary calculations
    ...
    */
    ...
    /**
    * \brief Method that inverts only the main diagonal matrices of matrix n11, thus avoiding unnecessary calculations
    * \param n11 : Matrix N11 to be inverted
    */
    void setInverseN11(Matrix n11);

    /**
    * \brief Method that inverts only the main diagonal matrices of matrix n22, thus avoiding unnecessary calculations
    ...
    */
    ...
    /**
    * \brief Auxiliary method that returns the residuals of a photogrammetric point, if the point is not photogrammetric it returns a matrix with zeroed elements
    ...
    */
    ...
    /**
    * \brief Method that returns the matrix with the initial approximations of the exterior orientation parameters of all images
    ...
    */
    ...
    /**
    * \brief Method that returns the list of photogrammetric points used in the adjustment calculation
    * \return std::deque<Point*> : List with Point objects of photogrammetric points
    */
    std::deque<Point*> getPhotogrammetricList();

    /**
    * \brief Method that checks if the user has set all parameters of the exterior orientation as initial values for all images
    * \return bool : Informs whether all images have initial values for the calculation of exterior orientation parameters
    */
    ...
    * \param hemi : integer informed in which hemisphere are the coordinates; in. -1=South +1=North
