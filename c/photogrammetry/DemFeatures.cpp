...
FeatureClass *DemFeatures::getFeatureClass(int classid) {
  if (classid < 1 || unsigned(classid) > feature_classes.size()) return NULL;
...
int DemFeatures::exportTxtFeatures(char *filename) {
  ...
  for (unsigned int i = 0; i < features.size(); i++) {
    ...
    for (unsigned int k = 0; k < df.points.size(); k++) {
      arq << "  " << k + 1 << ", " << df.points.at(k).X << ", "
          << df.points.at(k).Y << ", " << df.points.at(k).Z << "\n";
    }
...
bool DemFeatures::hasFeatureClass(FeatureClass *fc) {
  for (unsigned int i = 0; i < features.size(); i++) {
    if (getFeatureClass(features.at(i).feature_class) == fc) return true;
  }
  return false;
}
...
int DemFeatures::createShapefile(FeatureClass *fc, OGRSpatialReference *oSRS,
  ...
  switch (fc->type) {
    // It would be preferable to use wkt[TheGeomType]Z (with integer values
    // ​​1001, 1 002 and 1003 for pointz, linestringz and polygons
    // respective types), but in the development version of GDAL/OGR this was
    // not found!
    case 1:
  ...
  OGRLayer *poLayer =
      poDS->CreateLayer(fc->name.c_str(), oSRS, nShapeType, nullptr);
  if (poLayer == nullptr) {
...
DemFeature DemFeatures::getFeature(int feat) {
  // Create empty object
  DemFeature df;
  if (feat < 1 || unsigned(feat) > features.size()) return df;
...
DemFeature *DemFeatures::getFeatureLink(int feat) {
  // Create empty object
  if (feat < 1 || unsigned(feat) > features.size()) return NULL;
...
int DemFeatures::deleteFeature(int featid) {
  if (featid < 1 || unsigned(featid) > features.size()) return 0;
...
void DemFeatures::addNewPoint(int featid, int pointid, double X, double Y,
  ...
  if (unsigned(pointid) > features.at(featid - 1).points.size()) {
    ...
    features.at(featid - 1)
        .points.insert(features.at(featid - 1).points.begin() + pointid - 1,
                       dfp);
  }
...
int DemFeatures::deletePoint(int featid, int pointid) {
  ...
  features.at(featid - 1)
      .points.erase(features.at(featid - 1).points.begin() + pointid - 1);
...
int DemFeatures::getFeatureClassId(int featid) {
  if (featid < 1 || unsigned(featid) > features.size()) return -1;
...
void DemFeatures::calculateFeaturesAttributes() {
  for (unsigned int i = 0; i < features.size(); i++) {
    ...
    if (features.at(i).flag_calc_centroid) continue;
...
int DemFeatures::getNearestFeature(double X, double Y, double Z) {
  if (features.size() < 1) return 0;

  int best_feat = 1;
  double dist, best_dist = sqrt(pow(X - features.at(0).centroid.X, 2) +
                                pow(Y - features.at(0).centroid.Y, 2) +
                                pow(Z - features.at(0).centroid.Z, 2));
  double dist_k;

  for (unsigned int i = 1; i < features.size(); i++) {
    dist = sqrt(pow(X - features.at(i).centroid.X, 2) +
                pow(Y - features.at(i).centroid.Y, 2) +
                pow(Z - features.at(i).centroid.Z, 2));

    // If line, set nearest point as distance
    if (features.at(i).feature_type == 2) {
      if (features.at(i).points.size() < 1) break;

      dist = sqrt(pow(X - features.at(i).points.at(0).X, 2) +
                  pow(Y - features.at(i).points.at(0).Y, 2) +
                  pow(Z - features.at(i).points.at(0).Z, 2));

      for (unsigned int k = 1; k < features.at(i).points.size(); k++) {
        dist_k = sqrt(pow(X - features.at(i).points.at(k).X, 2) +
                      pow(Y - features.at(i).points.at(k).Y, 2) +
                      pow(Z - features.at(i).points.at(k).Z, 2));
...
int DemFeatures::getNearestPoint(int fid, double X, double Y, double Z) {
  if (fid < 1 || unsigned(fid) > features.size()) return 0;
...
int DemFeatures::loadFeatSp165(char *filename, bool append = false) {
  ...
  while ((!flag) && (!arq.fail()) && (count < 20)) {
    getline(arq, tag);
    tag = tag.substr(0, 16);
    if (tag.compare("<EFOTO_FEATURES>") == 0) flag = true;
    count++;
  }
  ...
  while (!arq.fail()) {
    ...
    tag = tag.substr(0, 17);
    if ((tag.compare("</EFOTO_FEATURES>") == 0) || (arq.fail())) break;
    ...
    if (tag.substr(0, 5).compare("Point") == 0) df.feature_type = 1;
    if (tag.substr(0, 4).compare("Line") == 0) df.feature_type = 2;
    if (tag.substr(0, 7).compare("Polygon") == 0) df.feature_type = 3;
  ...
  while ((!flag) && (!arq.fail()) && (count < 20)) {
    getline(arq, tag);
    tag = tag.substr(0, 14);
    if (tag.compare("<EFOTO_POINTS>") == 0) flag = true;
    count++;
  }
  ...
  while (!arq.fail()) {
    ...
    tag = tag.substr(0, 15);
    if ((tag.compare("</EFOTO_POINTS>") == 0) || (arq.fail())) break;
    ...
    if (point_id != 0) {
      ...
      for (unsigned int i = 0; i < 7; i++) getline(arq, tag);
    }
  }
...
void DemFeatures::showFeatures(bool full = false) {
  ...
  for (unsigned int i = 0; i < features.size(); i++) {
    ...
    if (full) {
      for (unsigned int k = 0; k < df.points.size(); k++) {
        printf("  %f, %f, %f\n", df.points.at(k).X, df.points.at(k).Y,
               df.points.at(k).Z);
      }
    }
    printf(" Centroid: %f, %f, %f\n", df.centroid.X, df.centroid.Y,
           df.centroid.Z);
    printf(" Perimeter: %f\n", df.perimeter);
    printf(" Area: %f\n", df.area);
...
void DemFeatures::addFeaturesToPairList(MatchingPointsList *mpl,
  ...
  for (unsigned int f = 0; f < features.size(); f++) {
    ...
    if (!usePolygons && df->feature_type == 3) continue;
...
void DemFeatures::addPolygonToMap(int feat_id, Matrix *map, double Xi,
  ...
  for (unsigned int i = 1; i <= polMap.getRows(); i++) {
    for (unsigned int j = 1; j <= polMap.getCols(); j++) {
      if (polMap.get(i, j) - 0.0 < 0.000000000001) continue;
...
Matrix DemFeatures::mapPolygon(int feat_id, double res_x, double res_y) {
  ...
  if (feat_id < 1 || unsigned(feat_id) > features.size()) return pmap;

  // Only polygons
  if (features.at(feat_id - 1).feature_type != 3) return pmap;
  ...
  for (int i = 0; i < pmap_height; i++) {
    for (int j = 0; j < pmap_width; j++) {
      ...
      if (!(isInside(feat_id, X, Y))) continue;
...
double DemFeatures::interpolateXYPolygon(int feat_id, double X, double Y,
                                         double D0 = -1.0) {
  if (feat_id < 1 || unsigned(feat_id) > features.size()) return 0.0;
...
void DemFeatures::calculateBoundingBox(int feat_id, double &Xi, double &Yi,
                                       double &Xf, double &Yf) {
  if (feat_id < 1 || unsigned(feat_id) > features.size()) return;
...
bool DemFeatures::isInside(int feat_id, double X, double Y) {
  if (feat_id < 1 || unsigned(feat_id) > features.size()) return false;
  ...
  for (unsigned int i = 0; i < n; i++) {
    angle += angle2D(df.points.at(i).X - X, df.points.at(i).Y - Y,
                     df.points.at((i + 1) % n).X - X,
                     df.points.at((i + 1) % n).Y - Y);
  }

  if (fabs(angle) < M_PI) return false;
...
std::string DemFeatures::getFeaturesList() {
  ...
  for (unsigned int i = 0; i < features.size(); i++) {
    ...
    txt << "Feature #" << i + 1 << "\t" << getFeatureTypeName(df.feature_type)
        << ", " << getFeatureClass(df.feature_class)->name << ", " << df.name
        << "\n";

    for (unsigned int k = 0; k < df.points.size(); k++) {
      txt << "\tPoint #" << k + 1 << "\tX=" << df.points.at(k).X
          << ", Y=" << df.points.at(k).Y << ", Z=" << df.points.at(k).Z << "\n";
    }
  }
...
std::string DemFeatures::getFeaturesToDisplay(int mode) {
  ...
  for (unsigned int i = 0; i < features.size(); i++) {
    ...
    for (unsigned int k = 0; k < df.points.size(); k++) {
      if (mode == 1) {
        txt << df.points.at(k).X << "\t" << df.points.at(k).Y << "\t"
            << df.points.at(k).Z << "\n";
      } else {
        txt << df.points.at(k).left_x << "\t" << df.points.at(k).left_y << "\t"
            << df.points.at(k).right_x << "\t" << df.points.at(k).right_y
            << "\n";
      }
    }
  }
