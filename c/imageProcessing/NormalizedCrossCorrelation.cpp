...
#include <stdio.h>
namespace {
double average(br::uerj::eng::efoto::Matrix* m,
                                           int row_i,
                                           int row_f,
                                           int col_i,
                                           int col_f)
...
double correlation(br::uerj::eng::efoto::Matrix* X,
                   br::uerj::eng::efoto::Matrix* Y,
                   int row_x_i,
                   int row_x_f,
                   int col_x_i,
                   int col_x_f,
...
double stddev(br::uerj::eng::efoto::Matrix* m,
              int row_i,
              int row_f,
              int col_i,
              int col_f)
        ...
        Matrix* searchmat)
{
    ...
    while (true) {
        ...
        if ((template_xi <= 0) || (template_yi <= 0) || (template_xf >= ref_w - 1)
                || (template_yf >= ref_h - 1)) {
    ...
    int search_window_xi = search_window_center_x - floor(double(
                               search_window_width) / 2.0),
                           search_window_yi = search_window_center_y - floor(double(
                                       search_window_height) / 2.0);
    int delta_tx = floor(double(template_width) / 2.0);
    int delta_ty = floor(double(template_height) / 2.0);

    for (int sw_i = search_window_yi ;
            sw_i <= search_window_yi + (search_window_height - 1); sw_i++) {
        for (int sw_j = search_window_xi ;
                sw_j <= search_window_xi + (search_window_width - 1); sw_j++) {
            ...
            p = correlation(refmat, searchmat, template_yi, template_yf, template_xi,
                            template_xf, sw_i, sw_i + (template_height - 1), sw_j,
                            sw_j + (template_width - 1));
        ...
        int template_cx, int template_cy, int matching_cx, int matching_cy)
{
    ...
    int delta_tx = round(double(template_width) / 2.0),
        delta_ty = round(double(template_height) / 2.0);
    int delta_tx_end = template_width - delta_tx,
        delta_ty_end = template_height - delta_ty;

    // Check if template is out of bounds
    if ((template_cx - delta_tx < 0) || (template_cy - delta_ty < 0)
    ...
    // Check if matching window is out of bounds
    if ((matching_cx - delta_tx < 0) || (matching_cy - delta_ty < 0)
            || (matching_cx + delta_tx_end > s_w) || (matching_cy + delta_ty_end > s_h)) {
        return 0.0;
    }

    int template_xi = 1 + template_cx - delta_tx,
        template_yi = 1 + template_cy - delta_ty;
    int template_xf = template_xi + (template_width - 1),
        template_yf = template_yi + (template_height - 1);
    int matching_window_xi = 1 + matching_cx - delta_tx,
        matching_window_yi = 1 + matching_cy - delta_ty;
    int matching_window_xf = matching_window_xi + (template_width - 1),
        matching_window_yf = matching_window_yi + (template_height - 1);
    double p = correlation(X, Y, template_yi, template_yf, template_xi, template_xf,
                           matching_window_yi, matching_window_yf, matching_window_xi, matching_window_xf);
    return p;
}
