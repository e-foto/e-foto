 ...
 */
LsmFlag LeastSquaresMatching::searchHomologous(Matrix* img1, Matrix* img2,
        double Tx, double Ty,
        double Mx, double My)
{
    ...
    if ((Tx - delta_tx <= 0) || (Ty - delta_ty <= 0)
            || (Tx + delta_tx_end >= ref_w - 1) || (Ty + delta_ty_end >= ref_h - 1)) {
        template_width = ori_template_width;
        template_height = ori_template_height;
        return LsmFlag::OutOfBonds;
    }

    // Check if matching window is out of bounds
    if ((Mx - delta_tx <= 0) || (My - delta_ty <= 0)
            || (Mx + delta_tx_end >= s_w - 1) || (My + delta_ty_end >= s_h - 1)) {
        template_width = ori_template_width;
        template_height = ori_template_height;
        return LsmFlag::OutOfBonds;
    }
    ...
    for (int it = 0; it < max_iterations; it++) {
        ...
        for (int l = -1; l <= template_height; l++) {
            for (int k = -1; k <= template_width; k++) {
                ...
                Maux.set(l + 2, k + 2,
                         ...
                         + img2->get(floor(y),
                         floor(x) + 1)*delta_x * (1 - delta_y) + img2->get(floor(y) + 1,
                         floor(x))*delta_y * (1 - delta_x) + img2->get(floor(y) + 1,
                         floor(x) + 1)*delta_x * delta_y);
            }
        }
        ...
        if (it >= max_iterations - 1 && (t.get(1, 1) > over_it_distance
            ...
            template_width = ori_template_width;
            template_height = ori_template_height;
            return LsmFlag::MaxIterationsAchieved;
        }
    }
