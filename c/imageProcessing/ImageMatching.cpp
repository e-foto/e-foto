...
ImageMatching::ImageMatching() = default;
        ...
        MatchingPointsList* repository, MatchingPointsList* mpoints)
{
    ...
    switch (radiometric_mode) {
    case ImgRadioCorrection::Equalization:
        rt.histeq(img1, image_depth);
        rt.histeq(img2, image_depth);
        break;

    case ImgRadioCorrection::HistMatching:
        rt.histmatching(img1, img2, image_depth);
        break;

    case ImgRadioCorrection::NoCorrection:
        break;// Do Nothing
    }
                                   ...
                                   double sx, double sy)
{
    ...
    while (pop(lx, ly, rx, ry)) {
        // Map coordinates
        int i = (int(ly) / step_y) + 1;
        int j = (int(lx) / step_x) + 1;
        ...
        if (matching_method == MatchingMet::NCrossCorr) {
        ...
        if (matching_method == MatchingMet::LeastSqMatch) {
