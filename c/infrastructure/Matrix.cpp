...
Matrix::Matrix(unsigned int rows, unsigned int cols): nrows{rows}, ncols{cols}
{
...
Matrix Matrix::inverse2()
{
    ...
    if (nrows != ncols) {
        ...
        for (unsigned int i = 1; i <= nrows; i++) { //definindo a linha a trabalhar
            element = Inv.get(i, i);
            ...
            for (unsigned int k = 1; k <= nrows; k++) {
                if (k != i) {
                    element = Inv.get(k, i);
