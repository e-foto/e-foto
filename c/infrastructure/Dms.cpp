...
Dms::Dms(int degree, int minute, double second, bool newSignal):
    signal{newSignal}
{
    if (degree < 0) {
        setSignal(true);
...
{
    if ( (this->degree >= 0 && this->degree < 360) && (this->min >= 0
            && this->min < 60) && (this->sec >= 0.0 && this->sec < 60.0) ) {
        return true;
    }
    else {
        return false;
...
void Dms::setDms(const Dms& newDms)
{
