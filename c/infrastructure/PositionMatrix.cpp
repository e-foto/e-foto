...
PositionMatrix::PositionMatrix(unsigned int dimensions, std::string newUnit):
    nrows{dimensions},
    ncols{1}
{
...
PositionMatrix::PositionMatrix(const Matrix& anotherMatrix):
    nrows{anotherMatrix.getRows()},
    ncols{anotherMatrix.getCols()}
{
    nw(anotherMatrix.getRows(), anotherMatrix.getCols());
    *this = anotherMatrix; // Allocate matrix and fill its attributes.
}

PositionMatrix::PositionMatrix(const PositionMatrix& anotherMatrix):
    nrows{anotherMatrix.getRows()},
    ncols{anotherMatrix.getCols()}
{
    nw(anotherMatrix.getRows(), anotherMatrix.getCols());
...
size_t PositionMatrix::getCols() const
{
    return ncols;
}

size_t PositionMatrix::getRows() const
{
    return nrows;
}
        ...
        if (!arq.fail()) {
            ...
            if (!arq.fail()) {
                ...
                if (!arq.fail()) {
                    ...
                    for (unsigned int i = 0; (!arq.eof()) || (!arq.fail())
                            || (i < (cols * rows)); i++) {
...
int PositionMatrix::save(char* filename)
{
    ...
    if (!emfile.fail()) {
        emfile << "E-foto project double PositionMatrix Format" <<
...
{
    std::cout << "PositionMatrix " << nrows << 'x' << ncols << " (" << unit << ")"
              << std::endl;
    ...
    for (unsigned int i = 1; i <= nrows; i++) {
        ...
        {
            std::cout << std::setw(9) << std::setprecision(3) <<  get((unsigned int) i,
                      (unsigned int) j) << " ";
        }
...
bool PositionMatrix::operator ==(const Matrix& Par_Matrix)
{
    if ((nrows != Par_Matrix.nrows) || (ncols != Par_Matrix.ncols)) {
        ...
        for (unsigned int i = 0; i < (ncols * nrows); i++)
            if (_Mat[i] - Par_Matrix._Mat[i] > 0.00000001
                    || _Mat[i] - Par_Matrix._Mat[i] < -0.00000001) {
                return 0;
            }
