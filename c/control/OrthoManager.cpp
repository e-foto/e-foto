...
OrthoUserInterface *OrthoManager::getInterface() { return myInterface; }
...
void OrthoManager::addImagesToForm() {
  ...
  for (unsigned i = 1; i <= listAllImages.size(); i++)
    oui->comboBox->addItem(
        "Image " +
        QString::fromStdString(listAllImages.at(i - 1)->getImageId()));
}
...
void OrthoManager::runAllOrthoTogether() {
  ...
  for (unsigned i = 0; i < listAllImages.size(); i++) {
    ...
    Cx.push_back(w / 2);
    Cy.push_back(h / 2);
    if (w > best_dist_inital) best_dist_inital = double(w);
  ...
  for (unsigned curr_image = 0; curr_image < listAllImages.size();
       curr_image++) {
    ...
    for (double Y = Yi; Y < Yf; Y += res_y) {
      for (double X = Xi; X < Xf; X += res_x) {
        ...
        if (best_img == static_cast<int>(curr_image)) {
          (best_img == -1) ? pixel = 0.0
                           : pixel = interp.interpolate(&img_m, best_col,
                                                        best_lin, inter_method);
...
void OrthoManager::runOrthoIndividual(int image) {
  if (image < 1 || image > static_cast<int>(listAllImages.size())) return;
...
int OrthoManager::orthoRectificationGeoTiff(char *filename, int option,
                                            double user_res_x,
                                            double user_res_y) {
  ...
  if (option == 1) {
    ...
    if (expos != std::string::npos) base_fname = base_fname.substr(0, expos);
...
int OrthoManager::orthoRectification(char *filename, int option,
                                     double user_res_x, double user_res_y) {
  // Create new orthoimage
  if (ortho != NULL) delete ortho;
...
void OrthoManager::returnProject() { manager->reloadProject(); }
