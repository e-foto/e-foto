...
DEMUserInterface *DEMManager::getInterface() { return myInterface; }
...
void DEMManager::returnProject() { manager->reloadProject(); }

Image *DEMManager::getImage(int id) {
  for (unsigned i = 0; i < listAllImages.size(); i++) {
    if (listAllImages.at(i)->getId() == id) return listAllImages.at(i);
  }
...
void DEMManager::setListPoint() {
  ...
  for (unsigned i = 0; i < allPoints.size(); i++) {
    ...
    if (point != nullptr) listAllPoints.push_back(point);
  }
}
    ...
    for (unsigned int j = 0; j < listAllImages.size(); j++) {
      for (unsigned int i = 0; i < allPoints.size(); i++) {
        std::string data =
            allPoints.at(i)
                .elementByTagAtt(
                    "imageCoordinates", "image_key",
                    Conversion::intToString(listAllImages.at(j)->getId()))
                .getContent();
        if (data != "") {
...
double DEMManager::getAngleBetweenImages(double X1, double Y1, double X2,
                                         double Y2) {
...
bool DEMManager::checkAnglesAlligned(double angle1, double angle2,
                                     double tolerance) {
...
void DEMManager::setLSMSettings(int _lsm_temp, int _lsm_it, double _lsm_th,
                                double _lsm_std, int _lsm_dist,
                                double _lsm_shift, double _lsm_shear,
...
void DEMManager::setCancel() {
  ...
  if (im != NULL) im->setCancel();

  if (grid != NULL) grid->setCancel();
}
...
int DEMManager::loadDem(char *filename, int fileType) {
  pairs.clear();
  if (pairs.load(filename, fileType) != 1) return 0;

  // If file type is 2D (option 1 or 2), must calculate 3D coords
  if (fileType == 1 || fileType == 2) calcPointsXYZ();
...
int DEMManager::loadDemFeature(char *filename) {
  ...
  // Stereoplotter 1.65
  bool dfFlag = df->loadFeatures(filename, false);
  DEMUserInterface_Qt *dui = (DEMUserInterface_Qt *)myInterface;
 ...
 * DEM extraction
 **/
void DEMManager::interpolateGrid(int method, int garea, double Xi, double Yi,
                                 double Xf, double Yf, double res_x,
                                 double res_y, int tsurface, double ma_exp,
                                 double ma_dist, int ma_weight,
                                 int gridSource) {
  double Zi, Zf;
  ...
  if (gridSource != 1) modPairs = pairs;

  if (gridSource != 0) {
    if (df != NULL) df->addFeaturesToPairList(&modPairs, false);
  }
...
int DEMManager::extractDEM(int option, bool clearMList) {
  ...
  if (pairs.size() > 0) {
    ...
    for (int i = 0; i < 6; i++) hist[i] = (100 * hist[i]) / pairs.size();
...
void DEMManager::extractDEMPair(int pair) {
  if (pair < 0 || pair > static_cast<int>(listPairs.size()) - 1) return;
  ...
  std::string filename =
      getImage(left_id)->getFilepath() + "/" + getImage(left_id)->getFilename();
  dui->loadImage(img1, (char *)filename.c_str(), downsample);
  dui->setStatus((char *)"Loading right image ...");
  filename = getImage(right_id)->getFilepath() + "/" +
             getImage(right_id)->getFilename();
  ...
  if (match_method == 0)  // 0 is NCC match method
  {
    corr_th = ncc_th;
    std = ncc_std;
  }
...
std::string DEMManager::getDemQuality(char *filename, int option) {
  ...
  if (option == 0) {
    if (!mpl.load(filename, MatchingPointsList::loadXYZ)) return "";
  } else {
    if (!df2.loadFeatures(filename, false)) return "";
    df2.addFeaturesToPairList(&mpl, true);
  }

  if (mpl.size() < 1) return "";

  return grid->calculateDemQuality(mpl);
}

double DEMManager::calculateDemRes(double ds) {
  if (listAllImages.size() < 1) return 0.0;
