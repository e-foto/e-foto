...
bool PTManager::exec()
{
    if (efotoManager != NULL && mySensor != NULL
            && listAllImages.size() > 1 && listOis.size() > 1) {
        qApp->processEvents();
...
int PTManager::getTerrainLatHemisphere()
{
    ...
    int hemi = terrain.elementByTagName("Lat").attribute("direction") == "S" ? -1 :
               1;
    return hemi;
}
...
{
    EDomElement terrain = efotoManager->getXml("terrain");
    int hemi = terrain.elementByTagName("Long").attribute("direction") == "W" ? -1 :
               1;
    return hemi;
}
