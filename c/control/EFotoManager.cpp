...
Terrain* EFotoManager::instanceTerrain() {
  if (theTerrain != NULL) return theTerrain;
  EDomElement root(xmlData);
  EDomElement xmlTerrain = root.elementByTagName("terrain");
  if (xmlTerrain.getContent().compare("") == 0) return NULL;
...
Sensor* EFotoManager::instanceSensor(int id) {
  for (unsigned int i = 0; i < sensors.size(); i++)
    if (sensors.at(i)->getId() == id) return sensors.at(i);
  EDomElement root(xmlData);
  EDomElement xmlSensor =
      root.elementByTagAtt("sensor", "key", Conversion::intToString(id));
  if (xmlSensor.getContent().compare("") == 0) return NULL;
  if (xmlSensor.elementByTagName("geometry").toString().compare("frame") == 0 &&
      xmlSensor.elementByTagName("calculationMode")
              .toString()
              .compare("With Fiducial Marks") == 0) {
    SensorWithFiducialMarks* newSensorWithFiducialMarks =
        new SensorWithFiducialMarks();
  ...
  if (xmlSensor.elementByTagName("geometry").toString().compare("frame") == 0 &&
      xmlSensor.elementByTagName("calculationMode")
              .toString()
              .compare("With Sensor Dimensions") == 0) {
    SensorWithKnowDimensions* newSensorWithKnowDimensions =
        new SensorWithKnowDimensions();
  ...
  if (xmlSensor.elementByTagName("geometry").toString().compare("frame") == 0 &&
      xmlSensor.elementByTagName("calculationMode")
              .toString()
              .compare("Fixed Parameters") == 0) {
    SensorWithKnowParameters* newSensorWithKnowParameters =
        new SensorWithKnowParameters();
...
Flight* EFotoManager::instanceFlight(int id) {
  for (unsigned int i = 0; i < flights.size(); i++)
    if (flights.at(i)->getId() == id) return flights.at(i);
  EDomElement root(xmlData);
  EDomElement xmlFlight =
      root.elementByTagAtt("flight", "key", Conversion::intToString(id));
  if (xmlFlight.getContent().compare("") == 0) return NULL;
...
Image* EFotoManager::instanceImage(int id) {
  for (unsigned int i = 0; i < images.size(); i++)
    if (images.at(i)->getId() == id) return images.at(i);
  EDomElement root(xmlData);
  EDomElement xmlImage =
      root.elementByTagAtt("image", "key", Conversion::intToString(id));
  if (xmlImage.getContent().compare("") == 0) return NULL;
...
void EFotoManager::instanceAllImages() {
  ...
  for (unsigned int i = 0; i < xmlAllImages.size(); i++) {
    qApp->processEvents();
    if (xmlAllImages.at(i).getContent().compare("") == 0) continue;
    bool notAvailable = true;
    for (unsigned int j = 0; j < images.size() && notAvailable; j++)
      if (images.at(j)->getId() ==
          Conversion::stringToInt(xmlAllImages.at(i).attribute("key")))
        notAvailable = false;
    if (notAvailable) {
      Image* newImage = new Image();
...
void EFotoManager::instanceAllPoints() {
  ...
  for (unsigned int i = 0; i < xmlAllPoint.size(); i++) {
    qApp->processEvents();
    if (xmlAllPoint.at(i).getContent().compare("") == 0) continue;

    // Evita reinstaciar pontos que ja estejam disponiveis
    bool notAvailable = true;
    for (unsigned int j = 0; j < points.size() && notAvailable; j++)
      if (points.at(j)->getId() ==
          Conversion::stringToInt(xmlAllPoint.at(i).attribute("key")))
        notAvailable = false;
    if (notAvailable) {
...
Point* EFotoManager::instancePoint(int id) {
  for (unsigned int i = 0; i < points.size(); i++)
    if (points.at(i)->getId() == id) return points.at(i);
  EDomElement root(xmlData);
  EDomElement xmlPoint =
      root.elementByTagAtt("point", "key", Conversion::intToString(id));
  if (xmlPoint.getContent().compare("") == 0) return NULL;
...
InteriorOrientation* EFotoManager::instanceIO(int imageId) {
  for (unsigned int i = 0; i < IOs.size(); i++)
    if (IOs.at(i)->getImageId() == imageId) return IOs.at(i);
  EDomElement root(xmlData);
  EDomElement xmlIO = root.elementByTagAtt("imageIO", "image_key",
                                           Conversion::intToString(imageId));
  if (xmlIO.getContent().compare("") == 0) return NULL;
...
void EFotoManager::instanceAllIOs() {
  ...
  for (unsigned int i = 0; i < xmlAllIOs.size(); i++) {
    qApp->processEvents();
    if (xmlAllIOs.at(i).getContent().compare("") == 0) continue;
    bool notAvailable = true;
    for (unsigned int j = 0; j < IOs.size() && notAvailable; j++)
      if (IOs.at(j)->getImageId() ==
          Conversion::stringToInt(xmlAllIOs.at(i).attribute("image_key")))
        notAvailable = false;
    // InteriorOrientation *pkj;
    // pkj->getImageId()
    if (notAvailable) {
      InteriorOrientation* newIO = new InteriorOrientation();
      newIO->setImage(image(
          Conversion::stringToInt(xmlAllIOs.at(i).attribute("image_key"))));
...
ExteriorOrientation* EFotoManager::instanceEO(int imageId) {
  for (unsigned int i = 0; i < EOs.size(); i++)
    if (EOs.at(i)->getImageId() == imageId) return EOs.at(i);
  EDomElement root(xmlData);
  EDomElement xmlEO = root.elementByTagAtt("imageEO", "image_key",
                                           Conversion::intToString(imageId));
  EDomElement xmlSR = root.elementByTagAtt("imageSR", "image_key",
                                           Conversion::intToString(imageId));
  if (xmlEO.getContent().compare("") == 0 ||
      xmlSR.getContent().compare("") == 0)
    return NULL;
...
void EFotoManager::instanceAllEOs() {
  ...
  for (unsigned int i = 0; i < xmlAllEOs.size(); i++) {
    qApp->processEvents();
    if (xmlAllEOs.at(i).getContent().compare("") == 0) continue;
    bool notAvailable = true;
    for (unsigned int j = 0; j < EOs.size() && notAvailable; j++)
      if (EOs.at(j)->getImageId() ==
          Conversion::stringToInt(xmlAllEOs.at(i).attribute("image_key")))
        notAvailable = false;
    if (notAvailable) {
      SpatialRessection* newEO =
          new SpatialRessection();  // SpatialRessection vai virar
                                    // ExteriorOrientation em breve.
      newEO->setImage(image(
          Conversion::stringToInt(xmlAllEOs.at(i).attribute("image_key"))));
...
void EFotoManager::deleteSensor(int id) {
  ...
  if (mySensor != NULL) {
    ...
    if (xmlSensor.elementByTagName("geometry").toString().compare("frame") ==
            0 &&
        xmlSensor.elementByTagName("platform").toString().compare("aerial") ==
            0 &&
        xmlSensor.elementByTagName("detector").toString().compare("film") ==
            0) {
      SensorWithFiducialMarks* mySensorWithFiducialMarks =
          (SensorWithFiducialMarks*)mySensor;
...
InteriorOrientation* EFotoManager::IO(int id) {
  for (unsigned int i = 0; i < IOs.size(); i++)
    if (IOs.at(i)->getImageId() == id) return IOs.at(i);
  return NULL;
}
...
ExteriorOrientation* EFotoManager::EO(int id) {
  for (unsigned int i = 0; i < EOs.size(); i++)
    if (EOs.at(i)->getImageId() == id) return EOs.at(i);
  return NULL;
}
 ...
 *
 */
std::string EFotoManager::getXml(std::string tagname, std::string att,
                                 std::string value) {
  EDomElement root(xmlData);
 ...
 *
 */
std::string EFotoManager::objectType(void) { return "EFotoManager"; }

/**
 *
 */
std::string EFotoManager::objectAssociations(void) { return ""; }
...
// XML methods
//
void EFotoManager::xmlSetData(std::string xml) { xmlData = xml; }

std::string EFotoManager::xmlGetData() { return xmlData; }

// Other methods
//

void EFotoManager::setSavedState(bool state) { savedState = state; }
 ...
 *
 */
std::string EFotoManager::getInterfaceType() { return interfaceType; }
 ...
 *
 */
void EFotoManager::setNextImage(int newImage) { nextImage = newImage; }
...
bool EFotoManager::execSR(int id) {
  ...
  instanceAllPoints();
  spatialRessection = new SRManager(this, srTerrain, srSensor, srFlight,
                                    srImage, srIO, sr, points);
...
bool EFotoManager::execPT() {
  ...
  Sensor* ptSensor =
      instanceSensor(Conversion::stringToInt(sensor.attribute("key")));
...
void EFotoManager::execPTReport() {
  ...
  if (salvar.exec()) {
    ...
    pt_report.createReport(fileExport.toUtf8().data());
  }
...
void EFotoManager::getImagesId(int pos, int& left, int& right,
                               std::deque<int>& listPairs) {
  // Check pos
  left = -1;
  right = -1;
  if (pos < 0 || unsigned(pos) > listPairs.size() - 1) return;
...
bool EFotoManager::existPair(int& id1, int& id2, std::deque<int>& listPairs) {
  ...
  if (listPairs.size() == 0) return false;
  ...
  for (size_t i = 0; i < listPairs.size(); i++) {
    ...
    if (pair_id1 == id1 && pair_id2 == id2) return true;
  }
...
int EFotoManager::getPairs(std::deque<int>& listPairs) {
  ...
  // S = maxDim * img->getFlight()->getScaleDen()  / (img->getResolution() *
  // 0.0254);
  ...
  for (size_t i = 0; i < imagesSize; i++) {
    ...
    for (size_t j = i + 1; j < imagesSize; j++) {
      ...
      // average between the longitudinal and the transversal (or lateral)
      // overlaps
      double avg =
          (flight->getLongitudinalOverlap() + flight->getTransversalOverlap()) /
          2.0;
      if (overlap < avg) continue;

      // Assign image ids (in this case, the vector number - image id ranges
      // from 1-N
      id1 = i + 1;
      id2 = j + 1;
...
void EFotoManager::saveEPP() { project->execAutosave(); }
