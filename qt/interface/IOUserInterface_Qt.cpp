...
IOUserInterface_Qt::IOUserInterface_Qt(IOManager *manager, QWidget *parent,
                                       Qt::WindowFlags fl)
  ...
  QObject::connect(actionInterior_orientation, SIGNAL(triggered()), this,
                   SLOT(calculateIO()));
  QObject::connect(actionView_report, SIGNAL(triggered()), this,
                   SLOT(viewReport()));
  QObject::connect(actionDone, SIGNAL(triggered()), this, SLOT(OIdone()));
  // QObject::connect(actionSet_mark, SIGNAL(triggered()), this,
  // SLOT(activeSetMode())); QObject::connect(actionMove, SIGNAL(triggered()),
  // this, SLOT(activePanMode())); QObject::connect(actionZoom,
  // SIGNAL(triggered()), this, SLOT(activeZoomMode()));
  // QObject::connect(actionFit_view, SIGNAL(triggered()), this,
  // SLOT(fitView()));

  this->manager = manager;
  if (manager->interiorDone()) actionView_report->setEnabled(true);
...
void IOUserInterface_Qt::languageChange() { retranslateUi(this); }
...
void IOUserInterface_Qt::receiveMark(QPointF p) {
  ...
  imageView->getMarker()->insertMark(
      ...
      QString::number(table1->currentIndex().row() + 1), mark);

  points->setData(points->index(table1->currentIndex().row(), 2),
                  QString::number(p.x(), 'f', 2));
  points->setData(points->index(table1->currentIndex().row(), 3),
                  QString::number(p.y(), 'f', 2));
  points->setData(points->index(table1->currentIndex().row(), 4),
                  QVariant(true));
...
void IOUserInterface_Qt::init() {
  ...
  mark = new Marker(SymbolsResource::getCross(
      Qt::green, QSize(24, 24), 1,
...
bool IOUserInterface_Qt::viewReport() {
  ...
  QVBoxLayout *laLabelLayout =
      new QVBoxLayout();  // layout contendo o label e a tabela de La
  QVBoxLayout *mvcLaLabelLayout =
      new QVBoxLayout();  // layout contendo o label e a tabela de mvc(La)
  /**/
  /* novo*/
...
void IOUserInterface_Qt::OIdone() { close(); }
  ...
  if (calculationMode == 1) {
    ...
    for (int row = 0; row < numberOfMarks; row++) {
      ...
      for (unsigned int col = 0; col < markData.size(); col++) {
        QStandardItem *item =
            new QStandardItem(QString(markData.at(col).c_str()));
    ...
    for (int row = 0; row < points->rowCount(); row++) {
      if (points->item(row, 2)->text().isEmpty()) continue;
      QString pointName = QString::number(row + 1);
      QPointF location(points->item(row, 2)->text().toDouble(),
                       points->item(row, 3)->text().toDouble());
