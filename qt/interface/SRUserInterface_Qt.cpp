...
SRUserInterface_Qt::SRUserInterface_Qt(SRManager *manager, QWidget *parent,
                                       Qt::WindowFlags fl)
  ...
  // Tem que rever esse primeiro connect... o calculate da SR não é direto, tem
  // que abrir a tela pra escolher iterações e precisão.
  QObject::connect(actionSpatialRessection, SIGNAL(triggered()), this,
                   SLOT(calculateSR()));
  ...
  // QObject::connect(actionSet, SIGNAL(triggered()), this,
  // SLOT(activeSetMode())); QObject::connect(actionUnset, SIGNAL(triggered()),
  // this, SLOT(activeUnsetMode())); QObject::connect(actionMove,
  // SIGNAL(triggered()), this, SLOT(activePanMode()));
  // QObject::connect(actionZoom, SIGNAL(triggered()), this,
  // SLOT(activeZoomMode())); QObject::connect(actionFit, SIGNAL(triggered()),
  // this, SLOT(fitView()));

  // FlightDirectionDisabled!
  // QObject::connect(actionFlight, SIGNAL(triggered()), this,
  // SLOT(setFlight()));

  this->manager = manager;
  if (manager->exteriorDone()) actionTable->setEnabled(true);
...
void SRUserInterface_Qt::languageChange() { retranslateUi(this); }
...
void SRUserInterface_Qt::init() {
  ...
  markOn = new Marker(SymbolsResource::getTriangle(
      Qt::green, Qt::transparent, QSize(24, 24), 2,
      true));  // Personalizando as marcas. Que no futuro eu quero melhorar para
               // inserir uso de 2 ou 3 marcas de acordo com o tipo de ponto.
  markOff = new Marker(SymbolsResource::getTriangle(
      Qt::darkRed, Qt::transparent, QSize(24, 24), 2,
      true));  // Personalizando as marcas. Que no futuro eu quero melhorar para
               // inserir uso de 2 ou 3 marcas de acordo com o tipo de ponto.
  ...
  connect(imageView->getMarker(), SIGNAL(clicked(QPointF)), this,
          SLOT(receivePoint(QPointF)));
  // connect (oldImageView, SIGNAL(mousePressed()), this, SLOT(informState()));
  // connect (oldImageView, SIGNAL(markClicked(QPoint)), this,
  // SLOT(receivePoint(QPoint))); connect (oldImageView, SIGNAL(changed()),
  // this, SLOT(makeRepaint()));

  // REVER
  // connect (oldImageView, SIGNAL(flightDirectionClicked(QPoint)), this,
  // SLOT(receiveFlightDirection(QPoint)));
...
void SRUserInterface_Qt::receivePoint(QPointF p) {
  ...
  if (table1->model()->rowCount() == 0) {
    QMessageBox *alert = new QMessageBox(
        QMessageBox::Warning, "Points missing",
        "You need to add points in the project before performing this task.\n");
    alert->show();
    return;
  }

  imageView->getMarker()->insertMark(
      p, table1->currentIndex().row() + 1,
      points->data(points->index(table1->currentIndex().row(), 2)).toString(),
      markOn);

  points->setData(points->index(table1->currentIndex().row(), 7),
                  QString::number(p.x(), 'f', 2));
  points->setData(points->index(table1->currentIndex().row(), 8),
                  QString::number(p.y(), 'f', 2));
  points->setData(
      points->index(table1->currentIndex().row(), 9),
      QString::number(manager->pointToDetector(p.x(), p.y()).at(0), 'f', 2));
  points->setData(
      points->index(table1->currentIndex().row(), 10),
      QString::number(manager->pointToDetector(p.x(), p.y()).at(1), 'f', 2));
  points->setData(points->index(table1->currentIndex().row(), 11),
                  QVariant(true));
  points->item(table1->currentIndex().row(), 2)->setCheckState(Qt::Checked);

  measurePoint(
      points->data(points->index(table1->currentIndex().row(), 0)).toInt(),
      p.x(), p.y());
  ...
  if (item->column() == 2) {
    if (points->data(points->index(item->row(), 11)).toBool()) {
      if (item->checkState() == Qt::Checked) {
        manager->selectPoint(
            points->data(points->index(item->row(), 0)).toInt());
      } else {
        manager->unselectPoint(
            points->data(points->index(item->row(), 0)).toInt());
      }
...
bool SRUserInterface_Qt::calculateSR() {
  int iterations;
  double gnssPrecision, insPrecision;
  bool ok;

  iterations =
      QInputDialog::getInt(this, "Max. of iterations",
                           "Set max. number of iterations", 10, 1, 50, 1, &ok);
  if (!ok) return false;
  gnssPrecision = QInputDialog::getDouble(
      this, "Precision", "Set the precision for camera coordinates (meters)",
      0.001, 0.000001, 10, 6, &ok);
  if (!ok) return false;
  insPrecision = QInputDialog::getDouble(
      this, "Precision", "Set the precision for attitude angles (radians)",
      0.001, 0.000001, 1, 6, &ok);
  if (!ok) return false;
...
bool SRUserInterface_Qt::viewReport() {
  ...
  /* novo */
  QVBoxLayout *xalabelLayout =
      new QVBoxLayout();  // layout contendo o label e a tabela de Xa
  QVBoxLayout *mvclabelLayout =
      new QVBoxLayout();  // layout contendo o label e a tabela de mvc(Xa)
  /**/

  ETableWidget *myXaView = new ETableWidget();

  QStringList listaXa;
  listaXa << "X0" << "Y0" << "Z0" << QString::fromUtf8("φ")
          << QString::fromUtf8("ω") << QString::fromUtf8("κ");
  ...
  /**/
  QVBoxLayout *laLabelLayout =
      new QVBoxLayout();  // layout contendo o label e a tabela de Xa
  QVBoxLayout *mvcLaLabelLayout =
      new QVBoxLayout();  // layout contendo o label e a tabela de mvc(Xa)
  ...
  if (myLaView->rowCount() != 0) myTab->addTab(LaView, QString::fromUtf8("La"));
  ...
  QString aux1 = QString::fromUtf8(
      "<font size=5>σ<sub>0</sub><sup>2</sup>=V<sup>T</sup>PV/(n-m)");  // σ²
                                                                        // hexadecimal
                                                                        // 03C3
                                                                        // 0342//\sigma_{0}^{2}
                                                                        // =
  ...
  QString aux2 = QString::fromUtf8(
      "<font size=5>σ<sub>0</sub>");  // σ² hexadecimal 03C3
                                      // 0342//\sigma_{0}^{2} = frac{V^{T} * P *
                                      // V}{n - m}\F
  aux2 += "=";
  aux2 +=
      QString::number(sqrt(Conversion::stringToDouble(myValues.at(2))), 'f', 6);
  aux2 += " mm</font>";
...
void SRUserInterface_Qt::updateDisplayedPoints() {
  for (int row = 0; row < points->rowCount(); row++) {
    if (points->item(row, 7)->text().isEmpty() ||
        points->data(points->index(row, 1)).toString() == "false")
      continue;
    QString pointName = points->data(points->index(row, 2)).toString();
    QPointF location(points->item(row, 7)->text().toDouble(),
                     points->item(row, 8)->text().toDouble());
...
void SRUserInterface_Qt::EOdone() { close(); }
...
bool SRUserInterface_Qt::exec() {
  ...
  for (int row = 0; row < numberOfPoints; row++) {
    ...
    for (unsigned int col = 0; col < pointData.size(); col++) {
      QStandardItem *item =
          new QStandardItem(QString(pointData.at(col).c_str()));

      if (col == 2) item->setCheckable(true);
    ...
    if (pointData.at(1) == "false") table1->setRowHidden(row, true);

    // Check if point was measured and check current row - fix by Marcelo
    // Teixeira Silveira
    if ((!points->item(row, 7)->text().isEmpty()) &&
        (!points->item(row, 8)->text().isEmpty()) &&
        (!points->item(row, 9)->text().isEmpty()) &&
        (!points->item(row, 10)->text().isEmpty()))
  ...
  connect(points, SIGNAL(itemChanged(QStandardItem *)), this,
          SLOT(updateSelection(QStandardItem *)));

  this->show();
  LoadingScreen::instance().close();
  qApp->processEvents();
  // oldImageView = new ImageView(centralwidget);
  imageView->loadImage(QString::fromStdString(manager->getImageFile()));
  {
