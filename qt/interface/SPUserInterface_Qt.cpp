...
SPUserInterface_Qt::SPUserInterface_Qt(SPManager* manager, QWidget* parent,
                                       Qt::WindowFlags fl)
    : QMainWindow(parent, fl) {
  setupUi(this);

  // Connections
  QObject::connect(comboBox_3, SIGNAL(currentIndexChanged(int)), this,
                   SLOT(updateClass(int)));
  QObject::connect(comboBox, SIGNAL(currentIndexChanged(int)), this,
                   SLOT(onChangePair(int)));
  QObject::connect(comboBox_5, SIGNAL(currentIndexChanged(int)), this,
                   SLOT(setColorMaskLeft(int)));
  QObject::connect(comboBox_6, SIGNAL(currentIndexChanged(int)), this,
                   SLOT(setColorMaskRight(int)));
  QObject::connect(comboBox_7, SIGNAL(currentIndexChanged(int)), this,
                   SLOT(setReverseLensGlasses(int)));
  QObject::connect(comboBox_2, SIGNAL(currentIndexChanged(int)), this,
                   SLOT(onStereoModeChanged(int)));
  ...
  QObject::connect(actionExport, SIGNAL(triggered()), this,
                   SLOT(onExportButton()));
  ...
  QObject::connect(actionRemove, SIGNAL(triggered()), this,
                   SLOT(onRemoveButton()));
  QObject::connect(actionRemove_All, SIGNAL(triggered()), this,
                   SLOT(onRemoveAllButton()));
  QObject::connect(actionEnd, SIGNAL(triggered()), this,
                   SLOT(onCloseFeature()));

  QObject::connect(actionSelect, SIGNAL(triggered(bool)), this,
                   SLOT(onSelPtButton()));
  QObject::connect(actionInsert, SIGNAL(triggered(bool)), this,
                   SLOT(onAddPtButton()));
  QObject::connect(actionEdit, SIGNAL(triggered(bool)), this,
                   SLOT(onEditPtButton()));
  QObject::connect(actionRemove_Point, SIGNAL(triggered()), this,
                   SLOT(onRemovePtButton()));

  QObject::connect(treeView, SIGNAL(clicked(QModelIndex)), this,
                   SLOT(onFeatureListClicked(QModelIndex)));
...
bool SPUserInterface_Qt::exec() {
  ...
  connect(&viewer->getMarker(), SIGNAL(clicked(QPointF, QPointF)), this,
          SLOT(stereoClicked(QPointF, QPointF)));
  connect(&viewer->getMarker(), SIGNAL(mouseMoved(QPointF, QPointF)), this,
          SLOT(stereoMoved(QPointF, QPointF)));
  connect(viewer->getDisplay(), SIGNAL(resized(int, int)), this,
          SLOT(adjustFit(int, int)));
  ...
  viewer->addDockWidget(Qt::BottomDockWidgetArea, featureListDockWidget,
  Qt::Horizontal); viewer->addDockWidget(Qt::RightDockWidgetArea,
  generalOptionsDockWidget, Qt::Vertical);
  viewer->addDockWidget(Qt::RightDockWidgetArea, featureEditorDockWidget,
  Qt::Vertical); viewer->tabifyDockWidget(generalOptionsDockWidget,
  featureEditorDockWidget); statusBar()->setHidden(true);
  */
  ...
  viewer->setCorner(Qt::BottomRightCorner, Qt::RightDockWidgetArea);
  viewer->addDockWidget(Qt::RightDockWidgetArea, featureListDockWidget,
                        Qt::Vertical);
  viewer->addDockWidget(Qt::RightDockWidgetArea, generalOptionsDockWidget,
                        Qt::Vertical);
  viewer->addDockWidget(Qt::RightDockWidgetArea, featureEditorDockWidget,
                        Qt::Vertical);
...
void SPUserInterface_Qt::updateTable() {
  ...
  treeView->setCurrentIndex(
      tree->index(pt_id - 1, 0, tree->index(feat_id - 1, 0)));
...
void SPUserInterface_Qt::onLoadButton() {
  ...
  QString filename = QFileDialog::getOpenFileName(
      this, tr("Open Features file"), lastDir,
      tr("Stereoplotter features file (*.spf);; All files (*.*)"));
  ...
  if (manager->getNumFeatures() > 0) {
    ...
    QAbstractButton* clearButton =
        msgBox.addButton("Clear", QMessageBox::YesRole);
    QAbstractButton* appendButton =
        msgBox.addButton("Append", QMessageBox::NoRole);
    msgBox.exec();
  ...
  bool sp_load_flag = manager->loadFeatures(filename.toUtf8().data(), append);
...
void SPUserInterface_Qt::onSaveButton() {
  ...
  //
  QFileDialog salvar(
      this, tr("Save features file"), lastDir,
      tr("Stereoplotter features file (*.spf);; All files (*.*)"));
  ...
  if (salvar.exec()) {
    ...
    manager->saveFeatures(filename.toUtf8().data());
  }
}

void SPUserInterface_Qt::onExportButton() {
  // File open dialog
  QFileDialog salvar(this, tr("Export features"), lastDir,
                     tr("Text file (*.txt);; Shape file (*.shp)"));
  salvar.setAcceptMode(QFileDialog::AcceptSave);
  ...
  if (salvar.exec()) {
    ...
    if (mode.contains("Text file (*.txt)")) {
      ...
      manager->exportFeatures(filename.toUtf8().data(), 0);
    } else {
      // manager->exportFeatures((char *)filename.toStdString().c_str(), 1);
      manager->exportFeatures(filename.toUtf8().data(), 1);
    }
  }
}
...
void SPUserInterface_Qt::onAddButton() {
  ...
  if (addDialog.exec() == QDialog::Accepted) {
    ...
    // manager->addFeature(nameEdit->text().toStdString(),
    // comboBox_3->currentIndex()+1,
    // (comboBox_4->currentIndex()+1)%comboBox_4->count());
...
void SPUserInterface_Qt::onRemoveAllButton() {
  if (QMessageBox::warning(this, "Erase all?", "Erase all features?",
                           QMessageBox::Yes,
...
void SPUserInterface_Qt::onAddPtButton() {
  ...
  if (actionEdit->isChecked()) actionEdit->setChecked(false);

  if (actionSelect->isChecked()) actionSelect->setChecked(false);
...
void SPUserInterface_Qt::onEditPtButton() {
  viewer->getToolBar()->changeMode(1);
  // viewerSeparated->getToolBar()->changeMode(1);
  if (actionInsert->isChecked()) actionInsert->setChecked(false);
...
void SPUserInterface_Qt::refreshFeatureSelectedData() {
  ...
  manager->getSelectedFeatureData(sel_feat, fname, ftype, fclass, no_points,
                                  perimeter, area);
  ...
  comboBox_4->setCurrentIndex(fclass - 1 >= 0 ? fclass - 1
                                              : comboBox_4->count() - 1);
...
void SPUserInterface_Qt::stereoClicked(QPointF lPos, QPointF rPos) {
  if (measure_mode == 0) return;

  int fid, pid;
  manager->getSelected(fid, pid);

  if (measure_mode != 3) {
    if (fid < 1 || (pid < 1 && measure_mode != 1)) return;
  }
...
void SPUserInterface_Qt::centerImages(ObjectSpaceCoordinate coord,
                                      double zoom) {
  ...
  QPointF atR(centerRight.getPosition().get(1),
              centerRight.getPosition().get(2));
  ...
  // msg->setText(QString::number(atL.x())+QString("x")+QString::number(atL.y())+QString("
  // ")+QString::number(atR.x())+QString("x")+QString::number(atR.y()));
...
void SPUserInterface_Qt::changePair(int leftKey, int rightKey) {
  viewer->loadLeftImage(
      QString::fromStdString(manager->getFullImagePath(leftKey)));
  viewer->loadRightImage(
      QString::fromStdString(manager->getFullImagePath(rightKey)));
  viewer->update();
}
...
void SPUserInterface_Qt::onChangePair(int pos) {
  ...
  viewer->getDisplay()->adjustFit(
      QPointF(manager->getLeftPoint(manager->getBoundingBoxCenter())
                  .getPosition()
                  .get(1),
              manager->getLeftPoint(manager->getBoundingBoxCenter())
                  .getPosition()
                  .get(2)),
      QPointF(manager->getRightPoint(manager->getBoundingBoxCenter())
                  .getPosition()
                  .get(1),
              manager->getRightPoint(manager->getBoundingBoxCenter())
                  .getPosition()
                  .get(2)),
      manager->getBoundingBoxIdealZoom(viewer->getDisplay()->width(),
                                       viewer->getDisplay()->height()));
  updateData();
}
  ...
  viewer->getDisplay()->adjustFit(
      ...
      QPointF(manager->getRightPoint(manager->getBoundingBoxCenter())
              ...
              manager->getRightPoint(manager->getBoundingBoxCenter())
                  .getPosition()
                  .get(2)),
      manager->getBoundingBoxIdealZoom(width, height));
}
...
void SPUserInterface_Qt::setColorMaskLeft(int option) {
  viewer->getDisplay()->setColorMaskLeft(
      color_map[option][0], color_map[option][1], color_map[option][2]);
  updateData();
}

void SPUserInterface_Qt::setColorMaskRight(int option) {
  viewer->getDisplay()->setColorMaskRight(
      color_map[option][0], color_map[option][1], color_map[option][2]);
  updateData();
}
...
void AddDialog::accept() {
  ...
  manager->addFeature(nameEdit->text().toUtf8().data(),
                      typeCombo->currentIndex() + 1,
                      (classCombo->currentIndex() + 1) % classCombo->count());
  done(QDialog::Accepted);
}

void AddDialog::reject() { done(QDialog::Rejected); }
