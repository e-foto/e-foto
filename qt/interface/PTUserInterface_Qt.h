 ...
 */
class PTUserInterface_Qt : public QMainWindow,
                           public Ui::PTMainWindow,
                           public PTUserInterface {
  Q_OBJECT

 protected:
  static PTUserInterface_Qt *ptInst;
  PTUserInterface_Qt(PTManager *manager, QWidget *parent = 0,
  ...
  QStringList listImageLeft;   // contem a lista das imagens que podem ser
                               // escolhidas para a leftView
  QStringList listImageRight;  // contem a lista das imagens que podem ser
                               // escolhidas para a rightView
 ...
 protected:
   ...
   * \param event : Objeto contendo informaÃ§oes sobre o evento de fechamento de
   * janela
   */
 ...
 public:
   ...
   * \param image : string
   */
  void updateImageTable(ETableWidget *imageTable, std::string imageFilename,
                        bool move = true);
  bool exec();
 ...
 public slots:
  ...
  /**
   * \brief Metodo que mostra um widget com as informaÃ§oes principais do
   * resultado da fototriangulaÃ§ao
   */
  void viewReport();

  /**
   * \brief Metodo que mostra um widget com algumas opÃ§oes para rodar o calculo
   * da fotoTriangulaÃ§ao
   */
  ...
  /**
   * \brief Metodo para salvar as marcas e os resultados calculados pelo calculo
   * da fototriangulaÃ§ao
   */
  void acceptResults();
  ...
  /**
   * \brief Metodo que abre um widget parao usuario informar a direÃ§ao de voo
   * de cada imagem
   */
  ...
  /**
   * \brief Metodo que atualiza a tabela das imagens e garante que a mesma
   * imagem nÃ£o possa ser selecionada nos dois displays \param imageFilename :
   * Texto contendo o nome do arquivo de imagem
   */
  void updateImagesList(int index);
  ...
  /**
   * \brief Metodo que seleciona todas as linhas de todas as tabelas em que o
   * ponto parece \param pointKey : Key do ponto
   */
  void selectAllAppearances(int pointKey);

  /**
   * \brief Metodo que retorna o numero da linha em que o ponto aparece na
   * tabela, se o ponto nao estiver na tabela retorna -1 \param table : Objeto
   * tabela em que sera feita a busca \param pointKey : Key do ponto procurado
   * \return int : Valor da linha da tabela
   */
  int findKeyAppearances(ETableWidget *table, int pointKey);

  /**
   * \brief Metodo que altera a label e o widget que contem os nomes das imagens
   * em que o ponto aparece \param pointKey: Key do ponto
   */
  void showImagesAppearances(int pointKey);

  /**
   * \brief Metodo que cuida dos procedimentos necessarios para inserÃ§ao ou
   * update de um ponto na imagem \param pixel : Objeto que contem as
   * informaÃ§oes de linha e coluna do pixel
   */
  void imageClicked(QPointF pixel);

  /* Method into disuse:
   *
* \brief Metodo que atualiza os valores das coordenadas digitais dos objetos da
classe Point
* \param pixel : Objeto com as informaÃ§oes de linha e coluna do ponto
  ...
  */
  ...
  /**
   * \brief Metodo que reponsavel por atualizar no display a posição da marca de
   * um ponto \param display : Objeto display a ser atualizado \param imageKey:
   * Key da imagem contida no display \param pointKey: Key do ponto a ter sua
   * marca atualizada \param   pixel : Objeto contendo a informaÃ§ao sobre a
   * nova marcaÃ§ao do ponto no display
   */
  void updateMark(SingleDisplay *display, int imageKey, int pointKey,
                  QPointF pixel);
  ...
  /**
   * \brief Metodo que exporta os pontos e croqui do bloco para um arquivo xml
   * que o GoogleEarth interprete \param fromXML : indica se devem ser usadas a
   * orientações exteriores armazenadas no XML
   */
  void exportToKml(bool fromXML = false);

  /**
   * \brief Metodo que altera o numero maximo de iteraÃ§oes para o calculo da
   * fototriangulaÃ§ao \param iterations : Numero maximo de iteraÃ§oes
   */
  void setMaxIteration(int iterations);

  /**
   * \brief Metodo que altera o valor da convergencia dos parametros X0 Y0 e Z0
   * de cada imagem. \param value : Valor da convergencia em metros
   */
  void setMetricConvergencyValue(double value);

  /**
   * \brief Metodo que altera o valor da convergencia dos parametros omega phi e
   * kappa de cada imagem. \param value : Valor da convergencia em radianos
   */
  ...
  /**
   * \brief Metodo que ativa ou desativa o modo de inserÃ§ao de pontos em uma
   * imagem clicada \param insertionMode : Booleano para ativar ou desativar o
   * modo de inserÃ§ao
   */
  ...
  /**
   * \brief Metodo responsavel por tomar as providencias quando um usario clica
   * em algum item da tabela \param item : item da tabela clicado
   */
  ...
  /**
   * \brief Slot responsavel por comandar a medição automatica de pontos em
   * imagens cujos parametros iniciais foram fornecidos
   */
  ...
  /**
   * \brief Slot responsavel por providenciar a remoção de um conjunto de
   * medições
   */
  void removeAllMeasurements();

  /**
   * \brief Metodo que mostra, coloca em um widget, o resultado de calculo de OE
   * presente no corrente Xml
   */
  void showReportXml();

  void FlightFormClosed(QList<int> list);
  // Deixa o usuario entrar com o valor da linha e coluna na mao atualizando na
  // interface Atualmente desabilitada
 ...
 protected slots:
  ...
  /**
   * \brief Metodo que empilha as marcaÃ§oes para que possam ser recuperado pelo
   * undo \param oldCol   : Valor da coluna do ponto \param oldLin   : Valor da
   * linha do ponto \param pointKey : Key do ponto \param imageKey : Key da
   * imagem em o ponto esta \param pointId  : texto contendo o id do ponto
   */
  void putInStack(int oldCol, int oldLin, int pointKey, int imageKey,
                  QString pointId);

  /**
   * \brief MÃ©todo que substitui todos os "-1" para -- indicando que esse ponto
   * ainda nÃ£o foi marcado na imagem \param imageTable: Tabela com valores a
   * serem substituidos
   */
...
class PointMark {
 ...
 public:
   ...
   */
  explicit PointMark(QPointF coord, int keypoint, int keyImage,
                     QString id = "PointId");
   ...
   * \return QPointF : Obejto contendo informaÃ§oes sobre as coordenadas
   * digitais
   */
  ...
  /**
   * \brief Metodo que retorna um texto contendo os valores dos atributos do
   * objeto \return QString : Texto informando os valores dos atributos
   */
  QString toString();
};
