    ...
    : QWidget(parent, fl)
{
    ...
    downReslabel->setText(QString::number(manager->calculateDemRes(
            doubleSpinBox0->value()), 'f', 2) + " meters");
    allow_close = true;
...
void DEMUserInterface_Qt::closeEvent(QCloseEvent* e)
{
    ...
    if (manager->isDemUnsaved()) {
        if (QMessageBox::warning(this, "Unsaved work",
                                 "A DSM was generated and not saved. Continue?", QMessageBox::Ok,
...
void DEMUserInterface_Qt::showImage3D(Matrix* image, double xi, double dx,
                                      double yi, double dy, double zi, double dz, bool isGrayscale)
{
    SingleViewer* sv = new SingleViewer(this);
...
void DEMUserInterface_Qt::onDemSaveClicked()
{
    ...
    if (salvar.exec()) {
        ...
        // Save DEM
        manager->saveDem((char*)filename.toLocal8Bit().constData(),
...
void DEMUserInterface_Qt::onDemLoadClicked()
{
    ...
    if (!manager->loadDem((char*)filename.toLocal8Bit().constData(),
        ...
        QMessageBox::critical(this, "Load error",
                              "Error while loading file. Check if file format option matches the file.");
        return;
    }
...
void DEMUserInterface_Qt::onDemGridLoadClicked()
{
    ...
    QString filename = QFileDialog::getOpenFileName(this, tr("Open DSM Grid file"),
                       lastDir, filter) ;
...
void DEMUserInterface_Qt::onDemGridClicked()
{
    ...
    manager->interpolateGrid(/*comboBox0->currentIndex(), */
        comboBox1->currentIndex(), comboBox2_2->currentIndex(),
        XilineEdit->text().toDouble(), YilineEdit->text().toDouble(),
        XflineEdit->text().toDouble(), YflineEdit->text().toDouble(),
        doubleSpinBox_8->value(), doubleSpinBox_9->value(), comboBox6->currentIndex(),
        doubleSpinBox15->value(), doubleSpinBox16->value(), comboBox7->currentIndex(),
...
void DEMUserInterface_Qt::setElapsedTime(double t, int opt)
{
    ...
    if (h > 0) {
        str_time = QString::number(h) + h_str;
    }

    if (h > 0 || m > 0) {
        str_time += QString::number(m) + m_str;
    }
...
void DEMUserInterface_Qt::setGridData(double Xi, double Yi, double Xf,
                                      double Yf, double Zi, double Zf, double res_x, double res_y, int width,
                                      int height)
{
...
void DEMUserInterface_Qt::onLoadPtsButtonClicked()
{
    ...
    int i = filename.lastIndexOf("/");
    lastDir = filename.left(i);
    textEdit->setText(QString::fromLocal8Bit(manager->getDemQuality((
                          char*)filename.toLocal8Bit().constData(), comboBox->currentIndex()).c_str()));
}
...
{
    ImgRadioCorrection irc;

    switch (comboBox3->currentIndex()) {
    case 0:
        irc = ImgRadioCorrection::NoCorrection;
        break;

    case 1:
        irc = ImgRadioCorrection::Equalization;
        break;

    case 2:
        irc = ImgRadioCorrection::HistMatching;
        break;
    default:
        irc = ImgRadioCorrection::NoCorrection;
        break;
    ...
    switch (comboBox4->currentIndex()) {
      case 0: mm = MatchingMet::NCrossCorr;
        break;
      case 1: mm = MatchingMet::LeastSqMatch;
        break;
      default: mm = MatchingMet::LeastSqMatch;
        break;
      }

    manager->setStdParameters(spinBox3_2->value(), spinBox3_3->value(),
                              spinBox11_2->value(), spinBox11_3->value(), checkBox_5->isChecked());
    manager->setAutoExtractionSettings(irc, mm,
                                       spinBox1->value(), doubleSpinBox0->value());
    manager->setLSMSettings(spinBox3->value(), spinBox4->value(),
                            doubleSpinBox5->value(), doubleSpinBox6->value(), spinBox7->value(),
                            doubleSpinBox8->value(), doubleSpinBox9->value(), doubleSpinBox10->value(),
    ...
    int DEMflag = manager->extractDEM(comboBox5->currentIndex(),
                                      checkBox_2->checkState());
...
void DEMUserInterface_Qt::onStereoplotterClicked()
{
    ...
    QString filename = QFileDialog::getOpenFileName(this,
                       tr("Open Stereoplotter file"), lastDir,
    ...
    QMainWindow(parent)
{
    ...
    connect(tableWidget, SIGNAL(currentCellChanged(int, int, int, int)), this,
            SLOT(onTableClicked()));
    Marker mark(SymbolsResource::getCross(Qt::yellow, QSize(24, 24),
                                          2)); // Create marks
    viewer->getLeftMarker().changeMarker(mark);
    viewer->getRightMarker().changeMarker(mark);
    setCentralWidget(viewer);
    // Create image marks
    mark_seeds = new Marker(SymbolsResource::getCross(Qt::yellow, QSize(24, 24),
                            2));
...
{
    QString filename;
    filename = QFileDialog::getOpenFileName(this, tr("Load seeds"), lastDir,
                                            tr("Seeds (*.txt);; All files (*.*)"));
...
void SeedEditorUserInterface_Qt::addMatchingPoints()
{
    ...
    pw.hide();
    pairsLabel->setText(QString::number(no_pairs) + "/" + QString::number(
                            pairs.size()));
}
...
void SeedEditorUserInterface_Qt::addSeedsAndTable()
{
    ...
    for (unsigned int i = 0; i < seeds.size(); i++) {
        ...
        (mp->left_x > 0
         || mp->left_y > 0) ? newItem = new QTableWidgetItem(tr("%1").arg(
                     mp->left_x)) : newItem = new QTableWidgetItem("");
        newItem->setFlags(Qt::ItemIsSelectable | Qt::ItemIsEnabled);
        tableWidget->setItem(no_seeds, 1, newItem);
        (mp->left_x > 0
         || mp->left_y > 0) ? newItem = new QTableWidgetItem(tr("%1").arg(
                     mp->left_y)) : newItem = new QTableWidgetItem("");
        newItem->setFlags(Qt::ItemIsSelectable | Qt::ItemIsEnabled);
        tableWidget->setItem(no_seeds, 2, newItem);
        (mp->right_x > 0
         || mp->right_y > 0) ? newItem = new QTableWidgetItem(tr("%1").arg(
                     mp->right_x)) : newItem = new QTableWidgetItem("");
        newItem->setFlags(Qt::ItemIsSelectable | Qt::ItemIsEnabled);
        tableWidget->setItem(no_seeds, 3, newItem);
        (mp->right_x > 0
         || mp->right_y > 0) ? newItem = new QTableWidgetItem(tr("%1").arg(
                     mp->right_y)) : newItem = new QTableWidgetItem("");
        newItem->setFlags(Qt::ItemIsSelectable | Qt::ItemIsEnabled);
    ...
    seedsLabel->setText(QString::number(no_seeds) + "/" + QString::number(
                            seeds.size()) + "  ");
}
