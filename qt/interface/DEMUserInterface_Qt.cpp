...
DEMUserInterface_Qt::DEMUserInterface_Qt(DEMManager *manager, QWidget *parent,
                                         Qt::WindowFlags fl)
    : QWidget(parent, fl) {
  setupUi(this);
  ...
  QObject::connect(demButton, SIGNAL(clicked()), this,
                   SLOT(onDemExtractionClicked()));
  QObject::connect(saveButton2, SIGNAL(clicked()), this,
                   SLOT(onDemSaveClicked()));
  QObject::connect(saveButton, SIGNAL(clicked()), this,
                   SLOT(onDemGridSaveClicked()));
  QObject::connect(loadButton, SIGNAL(clicked()), this,
                   SLOT(onDemLoadClicked()));
  QObject::connect(loadButton2, SIGNAL(clicked()), this,
                   SLOT(onDemGridLoadClicked()));
  QObject::connect(interButton, SIGNAL(clicked()), this,
                   SLOT(onDemGridClicked()));
  QObject::connect(checkBox, SIGNAL(stateChanged(int)), this,
                   SLOT(onLSMCheckChanged(/*int*/)));
  QObject::connect(abortButton, SIGNAL(clicked()), this,
                   SLOT(onAbortClicked()));
  QObject::connect(comboBox2_2, SIGNAL(currentIndexChanged(int)), this,
                   SLOT(onGridAreaLimitsStateChanged(int)));
  QObject::connect(comboBox1, SIGNAL(currentIndexChanged(int)), this,
                   SLOT(onInterStateChanged(int)));
  QObject::connect(checkBox_3, SIGNAL(stateChanged(int)), this,
                   SLOT(onShowImageStateChanged(int)));
  QObject::connect(seedsButton, SIGNAL(clicked()), this,
                   SLOT(onSeedsEditorClicked()));
  QObject::connect(stereoButton, SIGNAL(clicked()), this,
                   SLOT(onStereoplotterClicked()));
  QObject::connect(comboBox4, SIGNAL(currentIndexChanged(int)), this,
                   SLOT(onMatchingMethodChanged(int)));
  QObject::connect(loadPtsButton, SIGNAL(clicked()), this,
                   SLOT(onLoadPtsButtonClicked()));
  QObject::connect(saveQButton, SIGNAL(clicked()), this,
                   SLOT(onSavePtsButtonClicked()));
  QObject::connect(spinBox3, SIGNAL(valueChanged(int)), this,
                   SLOT(onLSMTemplateSizeChanged(int)));
  QObject::connect(spinBox11, SIGNAL(valueChanged(int)), this,
                   SLOT(onCorrTemplateSizeChanged(int)));
  QObject::connect(spinBox3_3, SIGNAL(valueChanged(int)), this,
                   SLOT(onLSMTemplateSizeChanged(int)));
  QObject::connect(spinBox11_3, SIGNAL(valueChanged(int)), this,
                   SLOT(onCorrTemplateSizeChanged(int)));
  QObject::connect(doubleSpinBox0, SIGNAL(valueChanged(double)), this,
                   SLOT(onDownValueChanged()));
  ...
  downReslabel->setText(
      QString::number(manager->calculateDemRes(doubleSpinBox0->value()), 'f',
                      2) +
      " meters");
...
void DEMUserInterface_Qt::languageChange() { retranslateUi(this); }
...
void DEMUserInterface_Qt::closeEvent(QCloseEvent *e) {
  ...
  if (manager->isDemUnsaved()) {
    if (QMessageBox::warning(this, "Unsaved work",
                             "A DSM was generated and not saved. Continue?",
                             QMessageBox::Ok,
  ...
  if (manager->isGridUnsaved()) {
    if (QMessageBox::warning(
            this, "Unsaved work",
            "An interpolated DSM Grid was generated and not saved. Continue?",
...
void DEMUserInterface_Qt::showImage2D(Matrix *image, double xi, double dx,
                                      double yi, double dy, bool isGrayscale) {
...
void DEMUserInterface_Qt::showImage3D(Matrix *image, double xi, double dx,
...
void DEMUserInterface_Qt::setAutoExtInfo(int nseeds, int nmatch, double min,
                                         double max) {
...
void DEMUserInterface_Qt::setBoundingBox(double Xi, double Yi, double Xf,
                                         double Yf) {
...
void DEMUserInterface_Qt::onDemSaveClicked() {
  ...
  if (salvar.exec()) {
    ...
    // lastDir, tr("DEM (*.pix);; Text file (*.txt);; All files (*.*)"));
    ...
    // Save DEM
    manager->saveDem(filename.toUtf8().data(), comboBox8->currentIndex());
  }
}
void DEMUserInterface_Qt::onDemGridSaveClicked() {
  ...
  if (salvar.exec()) {
    ...
    // Save DEM
    manager->saveDemGrid((char *)filename.toLocal8Bit().constData(),
                         comboBox9->currentIndex());
  }
}
void DEMUserInterface_Qt::onDemLoadClicked() {
  ...
  QString filename =
      QFileDialog::getOpenFileName(this, tr("Open DSM file"), lastDir, filter);
  ...
  // Load DEM
  if (!manager->loadDem((char *)filename.toLocal8Bit().constData(),
                        comboBox8->currentIndex())) {
    QMessageBox::critical(this, "Load error",
                          "Error while loading file. Check if file format "
                          "option matches the file.");
    return;
  }
...
void DEMUserInterface_Qt::onDemGridLoadClicked() {
  ...
  QString filename = QFileDialog::getOpenFileName(
      this, tr("Open DSM Grid file"), lastDir, filter);
  // if no file name written, return
  if (filename == "") return;
  // Save last dir
  int i = filename.lastIndexOf("/");
  ...
  // Load DEM Grid
  if (!manager->loadDemGrid((char *)filename.toLocal8Bit().constData(),
                            comboBox9->currentIndex())) {
    QMessageBox::critical(this, "Load error",
                          "Error while loading file. Check if file format "
                          "option matches the file.");
...
void DEMUserInterface_Qt::enableAfterDEM(int sender) {
  ...
  if (demSource < 3) {
    if (sender == 2) comboBox0->setCurrentIndex(1);
  } else {
...
void DEMUserInterface_Qt::onDemGridClicked() {
  if (!checkBoundingBox()) return;

  // Perform interpolation
  manager->interpolateGrid(
      /*comboBox0->currentIndex(), */ comboBox1->currentIndex(),
      comboBox2_2->currentIndex(), XilineEdit->text().toDouble(),
      YilineEdit->text().toDouble(), XflineEdit->text().toDouble(),
      YflineEdit->text().toDouble(), doubleSpinBox_8->value(),
      doubleSpinBox_9->value(), comboBox6->currentIndex(),
      doubleSpinBox15->value(), doubleSpinBox16->value(),
      comboBox7->currentIndex(), comboBox0->currentIndex());

  if (manager->cancelFlag()) return;
...
void DEMUserInterface_Qt::setElapsedTime(double t, int opt) {
  ...
  switch (opt) {
    case 1:
      ...
      break;
    default:
      matchingTimeLabel->setText(str_time);
  }
}
...
void DEMUserInterface_Qt::setGridData(double Xi, double Yi, double Xf,
                                      double Yf, double Zi, double Zf,
                                      double res_x, double res_y, int width,
                                      int height) {
...
int DEMUserInterface_Qt::checkBoundingBox() {
  ...
  if (Xi >= Xf || Yi >= Yf) {
    QMessageBox::critical(this, "Error",
                          "Error: grid bounding box has inconsistent values !");
    return 0;
  }
...
void DEMUserInterface_Qt::onLoadPtsButtonClicked() {
  ...
  textEdit->setText(QString::fromStdString(manager->getDemQuality(
      (char *)filename.toLocal8Bit().constData(), comboBox->currentIndex())));
}

void DEMUserInterface_Qt::onSavePtsButtonClicked() {
  // File open dialog
  QFileDialog salvar(this, tr("Save DSM quality report"), lastDir,
...
void DEMUserInterface_Qt::onDemExtractionClicked() {
  manager->setStdParameters(spinBox3_2->value(), spinBox3_3->value(),
                            spinBox11_2->value(), spinBox11_3->value(),
                            checkBox_5->isChecked());
  //        manager->setEliminateBadPoints(checkBox_4->isChecked()); // Works
  //        for images individually
  manager->setAutoExtractionSettings(
      comboBox3->currentIndex(), comboBox4->currentIndex(), spinBox1->value(),
      doubleSpinBox0->value());
  manager->setLSMSettings(spinBox3->value(), spinBox4->value(),
                          doubleSpinBox5->value(), doubleSpinBox6->value(),
                          spinBox7->value(), doubleSpinBox8->value(),
                          doubleSpinBox9->value(), doubleSpinBox10->value(),
                          checkBox->isChecked(), doubleSpinBox17->value());
  manager->setNCCSettings(spinBox11->value(), spinBox12->value(),
                          doubleSpinBox13->value(), doubleSpinBox14->value());
  int DEMflag =
      manager->extractDEM(comboBox5->currentIndex(), checkBox_2->checkState());

  if (!manager->cancelFlag() && DEMflag) enableAfterDEM(1);

  if (checkBox_4->isChecked()) manager->eliminateBadPoints();
}
...
void DEMUserInterface_Qt::loadImage(Matrix &I, char *filename, double sample) {
  ...
  for (unsigned int i = 1; i <= height; i++) {
    for (unsigned int j = 1; j <= width; j++) {
      ...
      pixel =
          (((pixel >> 16) & 0xFF) + ((pixel >> 8) & 0xFF) + (pixel & 0xFF)) / 3;
...
void DEMUserInterface_Qt::onStereoplotterClicked() {
  // File open dialog
  QString filename = QFileDialog::getOpenFileName(
      this, tr("Open Stereoplotter file"), lastDir,
      tr("Stereoplotter file (*.spf);; All files (*.*)"));
  // if no file name written, return
  if (filename == "") return;
...
void DEMUserInterface_Qt::showFatalErrorMessage(QString msg,
                                                bool abort = true) {
  QMessageBox::critical(this, "Error", msg);

  if (abort) doneButton->click();
}
...
SeedEditorUserInterface_Qt::SeedEditorUserInterface_Qt(DEMManager *manager,
                                                       QWidget *parent)
    : QMainWindow(parent) {
  setupUi(this);
  this->manager = manager;
  ...
  connect(comboBox1, SIGNAL(currentIndexChanged(int)), this,
          SLOT(onComboBox1Changed()));
  ...
  Marker mark(
      SymbolsResource::getCross(Qt::yellow, QSize(24, 24), 2));  // Create marks
...
void SeedEditorUserInterface_Qt::imageClicked(QPointF p) {
  ...
  if (sel_seeds.size() < 1) {
    QMessageBox::warning(this, "Warning",
                         "There is no seed pair selected to edit");
    return;
  }

  if (sel_seeds.size() > 1) {
    QMessageBox::warning(this, "Warning",
                         "There is are multiple seed pairs selected to edit");
    return;
  }
  ...
  if (sender() == &viewer->getLeftMarker()) {
    viewer->getLeftMarker().insertMark(p, key, QString::number(seed_id),
                                       mark_seeds);
    ...
    viewer->getRightMarker().insertMark(p, key, QString::number(seed_id),
                                        mark_seeds);
...
void SeedEditorUserInterface_Qt::onTableClicked(/*int row, int col*/) {
  ...
  if (int(x1) == 0 || int(y1) == 0 || int(x2) == 0 || int(y2) == 0) return;
...
void SeedEditorUserInterface_Qt::onRemoveButtonClicked() {
  ...
  if (sel_seeds.size() < 1) {
    QMessageBox::warning(this, "Warning",
                         "There is no seed pair selected to erase");
    return;
  }
...
void SeedEditorUserInterface_Qt::checkSelectedSeeds() {
  ...
  bool flag = false;
  if (no_pairs > 1) {
    if (selected.at(0)->row() == selected.at(1 * cols)->row()) flag = true;
  }
...
void SeedEditorUserInterface_Qt::saveSeeds() {
  QFileDialog salvar(this, tr("Save seeds"), lastDir,
                     tr("Seeds (*.txt);; All files (*.*)"));
  salvar.setAcceptMode(QFileDialog::AcceptSave);
  salvar.setDefaultSuffix("txt");
  if (salvar.exec()) {
    QString filename = salvar.selectedFiles()[0];
...
void SeedEditorUserInterface_Qt::loadSeeds() {
  ...
  filename = QFileDialog::getOpenFileName(
      this, tr("Load seeds"), lastDir, tr("Seeds (*.txt);; All files (*.*)"));
  if (filename == "") return;
...
void SeedEditorUserInterface_Qt::addMatchingPoints() {
  ...
  pairsLabel->setText(QString::number(no_pairs) + "/" +
                      QString::number(pairs.size()));
}
...
void SeedEditorUserInterface_Qt::addSeedsAndTable() {
  ...
  for (unsigned int i = 0; i < seeds.size(); i++) {
    ...
    if (mp->left_x > 0 || mp->left_y > 0)
      viewer->getLeftMarker().addMark(left_coord, key, QString::number(i + 1),
                                      mark_seeds);
    else
      viewer->getLeftMarker().addMark(left_coord, key, "", mark_empty);

    if (mp->right_x > 0 || mp->right_y > 0)
      viewer->getRightMarker().addMark(right_coord, key, QString::number(i + 1),
                                       mark_seeds);
    else
  ...
  seedsLabel->setText(QString::number(no_seeds) + "/" +
                      QString::number(seeds.size()) + "  ");
}
...
void SeedEditorUserInterface_Qt::updateData(/*int i*/) {
  ...
  // Update Images
  //
  std::string left_file = listAllImages.at(left_id - 1)->getFilepath() + '/' +
                          listAllImages.at(left_id - 1)->getFilename();
  std::string right_file = listAllImages.at(right_id - 1)->getFilepath() + '/' +
                           listAllImages.at(right_id - 1)->getFilename();
