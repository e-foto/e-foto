...
WindowsSelectPage::WindowsSelectPage(QString leftBox, QString rightBox,
                                     QWidget *parent)
    : QWizardPage(parent) {
...
void WindowsSelectPage::addSelectedItem() {
  if (uncheckedListView->currentIndex().isValid()) {
    ...
    checkedList.append(
        uncheckedModel->data(selectedItem, Qt::DisplayRole).toString());
    uncheckedList.removeAt(uncheckedList.indexOf(
        uncheckedModel->data(selectedItem, Qt::DisplayRole).toString()));
...
void WindowsSelectPage::removeSelectedItem() {
  if (checkedListView->currentIndex().isValid()) {
    ...
    uncheckedList.append(
        checkedModel->data(selectedItem, Qt::DisplayRole).toString());
    checkedList.removeAt(checkedList.indexOf(
        checkedModel->data(selectedItem, Qt::DisplayRole).toString()));
    uncheckedModel->setStringList(uncheckedList);
    checkedModel->setStringList(checkedList);
  }
...
std::deque<std::string> WindowsSelectPage::getSelectedItens() {
  ...
  for (int i = 0; i < size; i++)
    list.push_back(checkedModel->stringList().at(i).toUtf8().data());
  return list;
}
