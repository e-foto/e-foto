...
{
    //io= new Matrix(1,1);
    currentSpinBoxRow = currentSpinBoxColumn = currentDoubleSpinBoxColumn =
                            currentDoubleSpinBoxRow = -1;
...
ETableWidget::ETableWidget(Matrix values, char mode, int precision,
...
void ETableWidget::setTableData(Matrix values, char mode, int precision)
{
    ...
    for (int i = 0; i < rowCount(); i++) {
        for (int j = 0; j < columnCount(); j++) {
            QTableWidgetItem* temp = new QTableWidgetItem(QString::number(io->get(i + 1,
                    j + 1), getMode(), getDecimals()));
            temp->setTextAlignment(Qt::AlignCenter);
            temp->setFlags(Qt::ItemIsSelectable | Qt::ItemIsDragEnabled |
                           Qt::ItemIsEnabled);
...
void ETableWidget::resizeTable()
{
    ...
    if (columnCount() == 1) {
        setBaseSize(columnWidth(0) + widlinhas + verticalHeader()->width() + 15,
                    rowCount()*rowHeight(0) + heilinhas + horizontalHeader()->height());
    }
    else {
        setBaseSize(columnCount()*columnWidth(0) + widlinhas +
...
void ETableWidget::putIn(Matrix input, int row, int column, char mode,
                         int precision)
{
    setMode(mode);
    setDecimals(precision);
    ...
    for (unsigned int i = 0; i < input.getRows(); i++) {
        for (unsigned int j = 0; j < input.getCols(); j++) {
            QTableWidgetItem* temp = new QTableWidgetItem(QString::number(input.get(i + 1,
                    j + 1), getMode(), getDecimals()));
            temp->setTextAlignment(Qt::AlignCenter);
            temp->setFlags(Qt::ItemIsSelectable | Qt::ItemIsDragEnabled |
                           Qt::ItemIsEnabled);
...
{
    setRangeSelected(QTableWidgetSelectionRange(0, 0, rowCount() - 1,
                     columnCount() - 1), false);
}


void ETableWidget::putInColumn(QStringList list,
                               int column)//, QString type, bool enable, PositionMatrix *limits)
{
