...
DmsEdit::DmsEdit(QWidget *parent) : QLineEdit(parent) {
  // radValidator=new QDoubleValidator(-2*M_PI,2*M_PI,getDecimals(),this);
  // degValidator=new
  // QDoubleValidator(-359.9999999,359.9999999,getDecimals(),this);
...
int DmsEdit::getDecimals() { return decimals; }
void DmsEdit::validate() {
  ...
  if (getDmsEditMode() == DMS) {
    ...
    // qDebug("depois do string to dms:  %d  %d
    // %.4f",degMinSecLine->getDegree(),degMinSecLine->getMinute(),degMinSecLine->getSeconds());
    ...
    // qDebug()<<radVal;
    QRegularExpressionValidator *radValidator1 =
        new QRegularExpressionValidator(QRegularExpression(radVal), this);
    radValidator1->setLocale(QLocale::system());
    // radValidator1->locale().setNumberOptions(QLocale::O);
    // qDebug()<<radValidator1->locale().name()<<"simbolo decimal:
    // "<<radValidator1->locale().decimalPoint();
    ...
    // qDebug()<<degVal;
    QRegularExpressionValidator *degValidator1 =
        new QRegularExpressionValidator(QRegularExpression(degVal), this);
...
void DmsEdit::stepBy(int steps) {
  ...
  if (steps < 0) {
    for (int i = steps; i < 0; i++) stepDown();
  } else {
    for (int i = 0; i < steps; i++) stepUp();
  }
...
void DmsEdit::stepDown() {
  ...
  if (getDmsEditMode() == DMS) {
    ...
    if (positionValue() == DEGREES) {
      if (degMinSecLine->getDegree() == 0) degMinSecLine->setSignal(true);
...
void DmsEdit::stepUp() {
  ...
  if (getDmsEditMode() == DMS) {
    ...
    if (positionValue() == DEGREES) {
      ...
      if (degMinSecLine->getDegree() == 0) degMinSecLine->setSignal(false);
      // setText(degMinSecLine->toString(getDecimals()));
    } else if (positionValue() == MINUTES) {
 ...
 */
DmsEditMode DmsEdit::getDmsEditMode() { return mode; }
...
void DmsEdit::setDmsEditMode(DmsEditMode newMode) {
  ...
  if (getDmsEditMode() == DMS && newMode == RAD) {
    ...
    //	qDebug("radvalue =%f \tdegvalue=
    //%f\tdms=%s",radValue,degValue,degMinSecLine->toString(5).toStdString().c_str());
    ...
    //	qDebug()<<"deg to rad";
    //	qDebug("antes: radvalue =%f \tdegvalue=
    //%f\tdms=%s",radValue,degValue,degMinSecLine->toString(5).toStdString().c_str());
    radValue = Dms::degreeDecimalToRadiano(degValue);
    // qDebug("depois: radvalue =%f \tdegvalue=
    // %f\tdms=%s",radValue,degValue,degMinSecLine->toString(5));
...
void DmsEdit::updateValue(QString newValue) {
  ...
  // Dms
  // old(degMinSecLine->getDegree(),degMinSecLine->getMinute(),degMinSecLine->getSeconds(),degMinSecLine->hasSignal());
  ...
  if (newValue == "") {
    ...
    if (mode == DMS) {
      ...
      if (compMin >= 0 && compMax <= 0) {
        // qDebug("%s < %s <
        // %s",getMinimumDms()->toString(getDecimals()).toStdString().c_str(),degMinSecLine->toString(getDecimals()).toStdString().c_str(),getMaximumDms()->toString(getDecimals()).toStdString().c_str());
        ...
        degMinSecLine->setDms(*oldDegMinSecLine);
        // qDebug("%s não esta entre
        // ",oldDegMinSecLine->toString(getDecimals()).toStdString().c_str());
        setText(dmsToQString(degMinSecLine));
      }
      // setText(degMinSecLine->toString(getDecimals()));
      return;
...
double DmsEdit::getRadianValue() { return radValue; }

double DmsEdit::getDegreeValue() { return degValue; }

Dms *DmsEdit::getDmsValue() { return degMinSecLine; }

double DmsEdit::getMaximumDeg() { return degMax; }

double DmsEdit::getMinimumDeg() { return degMin; }

double DmsEdit::getMaximumRad() { return radMax; }

double DmsEdit::getMinimumRad() { return radMin; }

Dms *DmsEdit::getMaximumDms() { return dmsMax; }

Dms *DmsEdit::getMinimumDms() { return dmsMin; }

void DmsEdit::setDmsMaximum(int degree, int minutes, double seconds,
                            bool signal) {
...
void DmsEdit::setDmsMinimum(int degree, int minutes, double seconds,
                            bool signal) {
...
QString DmsEdit::dmsToQString(Dms *dms) {
  ...
  if (dms->hasSignal()) dmsQString.append(QString::fromUtf8("-"));
  ...
  dmsQString.append("\'");
  dmsQString.append(
      Conversion::doubleToString(dms->getSeconds(), getDecimals()).c_str());
  dmsQString.append("\"");
...
void DmsEdit::qStringToDms(QString dmsQString) {
  ...
  degMinSecLine->stringToDms(dmsQString.toUtf8().data());
