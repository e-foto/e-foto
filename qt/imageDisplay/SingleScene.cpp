...
SingleScene::SingleScene(QObject* parent, QString filepath)
    : AbstractScene(parent), viewpoint_(0, 0), viewportSize_(800, 600) {
  detailZoom_ = 4.0;
  if (filepath.isEmpty()) {
    rasterRsrc_ = NULL;
    minScale_ = 1;
...
SingleScene::~SingleScene() {
  if (rasterRsrc_ != NULL) delete (rasterRsrc_);
}

QPointF SingleScene::getViewpoint() { return viewpoint_; }

unsigned int SingleScene::getWidth() {
  if (rasterRsrc_ && rasterRsrc_->isValid()) return rasterRsrc_->width();
  return 0;
}

unsigned int SingleScene::getHeight() {
  if (rasterRsrc_ && rasterRsrc_->isValid()) return rasterRsrc_->height();
  return 0;
}
...
void SingleScene::centerContent() {
  if (rasterRsrc_ && rasterRsrc_->isValid()) viewpoint_ = rasterRsrc_->center();
}
...
void SingleScene::moveTo(QPointF pos) {
  ...
  if (rasterRsrc_ && rasterRsrc_->isValid()) {
    ...
    if (viewpoint_.x() < 0) viewpoint_.setX(0);
    if (viewpoint_.y() > rasterRsrc_->height())
      viewpoint_.setY(rasterRsrc_->height());
    if (viewpoint_.y() < 0) viewpoint_.setY(0);
  }
}
...
void SingleScene::scaleTo(double newScale, QPointF at) {
  ...
  if (at.x() < 0 || at.y() < 0 || !rasterRsrc_ || !(rasterRsrc_->isValid()))
    return;
  if (!((at.x() >= 0 && at.x() <= rasterRsrc_->width()) &&
        (at.y() >= 0 && at.y() <= rasterRsrc_->height())))
...
void SingleScene::setViewport(QSize viewportSize) {
  ...
  setLimitScale(viewportSize_.width() / (double)getWidth() <
                        viewportSize_.height() / (double)getHeight()
                    ? viewportSize_.width() / (double)getWidth()
                    : viewportSize_.height() / (double)getHeight(),
                getMaxScale());
  limitScale();
}
...
void SingleScene::transformImage(double H[9]) {
  if (rasterRsrc_ && rasterRsrc_->isValid()) rasterRsrc_->transformImage(H);
}
...
bool SingleScene::createImage(QSize size, QColor color) {
  ...
  rsrc->load(image);
  if (rsrc->isValid()) {
    if (rasterRsrc_) delete (rasterRsrc_);
    rasterRsrc_ = rsrc;
    viewpoint_ = QPointF(rasterRsrc_->center().toPoint());
    scale_ = 1;
    setLimitScale(viewportSize_.width() / (double)getWidth() <
...
double SingleScene::getMinScale() { return minScale_; }

double SingleScene::getMaxScale() { return maxScale_; }
...
QImage SingleScene::getImage() {
  if (!rasterRsrc_ || !(rasterRsrc_->isValid())) return QImage();
...
QImage SingleScene::getFrame(QSize targetSize, QSize rectSize) {
  ...
  QImage result(
      geometryRsrc_.draw(rasterRsrc_->getImageCut(targetSize, imageCut),
                         targetSize, viewpoint_, scale_));

  QPointF detailTopLeft =
      (detailViewpoint_ -
       QPointF(rectSize.width() / (detailZoom_ * scale_ * 2.0),
               rectSize.height() / (detailZoom_ * scale_ * 2.0)));
  QPointF toScreen = (detailTopLeft - viewpoint_) * scale_ +
                     QPointF(targetSize.width() / 2, targetSize.height() / 2);
  QRect detailedRect(
      toScreen.toPoint(),
...
QImage SingleScene::getThumb(QSize targetSize, QRect* rect) {
  ...
  double scale = thumbScale_ =
      newTargetSize.width() / static_cast<double>(rasterRsrc_->size().width());

  QImage result(
      geometryRsrc_.draw(rasterRsrc_->getImageCut(newTargetSize, imageCut),
                         newTargetSize, imageCut.center(), scale));

  if (rect) {
    QRect viewedRect(((viewpoint_ - QPointF(rect->width() / (scale_ * 2.0),
                                            rect->height() / (scale_ * 2.0))) *
                      scale)
                         .toPoint(),
...
QColor SingleScene::getColor(QPoint at) {
  if (!rasterRsrc_ || !(rasterRsrc_->isValid())) return QColor();
  return rasterRsrc_->getColor(at);
}

unsigned int SingleScene::getGrayColor(QPointF at, bool linear) {
  if (!rasterRsrc_ || !(rasterRsrc_->isValid())) return 0;
  return rasterRsrc_->getGrayColor(at.toPoint(), linear);
}
...
double SingleScene::getScale() { return scale_; }

double SingleScene::getThumbScale() { return thumbScale_; }

QPointF SingleScene::getDetailedPoint() { return detailViewpoint_; }

void SingleScene::setDetailedPoint(QPointF point) { detailViewpoint_ = point; }

double SingleScene::getDetailZoom() { return detailZoom_; }

void SingleScene::setDetailZoom(double zoom) { detailZoom_ = zoom; }
...
RasterResource* SingleScene::raster(/*int at*/) {
  // Num futuro em que existam vários rasterResources associados a uma cena o at
  // será o indice do raster buscado na cena O código -1 poderá ser usado para
  // indicar o raster corrente
  return rasterRsrc_;
}

GeometryResource* SingleScene::geometry(/*int at*/) {
  // Num futuro em que existam vários geometryResources associados a uma cena o
  // at será o indice do geometry buscado na cena O código -1 poderá ser usado
  // para indicar o geometry corrente
  return &geometryRsrc_;
}
