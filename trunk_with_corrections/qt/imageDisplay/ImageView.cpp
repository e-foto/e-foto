...
ImageView::ImageView(QString file, QWidget* parent) : SWidgetQt( parent )
{
	...
	this->createPin("myIOPin",QIcon(":/pin/Fiductial").pixmap(16,16).toImage());
	this->createPin("mySRPin1",QIcon(":/pin/ActivatedControl").pixmap(16,16).toImage());
	this->createPin("mySRPin2",QIcon(":/pin/DeactivatedControl").pixmap(16,16).toImage());
	this->createPin("mySRPin3",QIcon(":/pin/Arrow").pixmap(32,32).toImage());
...
void ImageView::drawFlightDirection(int x, int y)
{
	...
	QImage* aux = new QImage(QIcon(":/pin/Arrow").pixmap(32,32).toImage());
