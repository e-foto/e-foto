...
FeatureClass * Features::getFeatureClass(int classid)
{
 ...
 * 3- Polygon
 */
string Features::getFeatureTypeName(int ftype)
{
	switch (ftype)
	{
...
int Features::loadFeatures(char *filename, int mode, bool append=false)
{
	// 0- SP 1.65
...
Feature Features::getFeature(int feat)
{
	// Create empty object
	Feature df;
...
// Feature id ranges from 1-N
Feature* Features::getFeatureLink(int feat)
{
	// Create empty object
	if (feat < 1 || feat > features.size())
		return NULL;
...
// Returns feature id
int Features::addNewFeature(string name, string fdesc, int fclass, int ftype, int layer)
{
	Feature df;
	df.name = name;
...
int Features::deleteFeature(int featid)
{
...
int Features::copyFeature(int featid, double shift=0.0)
{
	// Check if feature is valid
	if (featid < 1 || featid > features.size())
		return 0;
...
void Features::addNewPoint(int featid, int pointid, double X, double Y, double Z)
{
	// Check if feature is valid
	if (featid < 1 || featid > features.size())
		return;
...
void Features::updatePoint(int featid, int pointid, double X, double Y, double Z)
{
	// Check if feature is valid
	if (featid < 1 || featid > features.size())
		return;
...
void Features::update2DPoint(int featid, int pointid, double lx, double ly, double rx, double ry)
{
	// Check if feature is valid
	if (featid < 1 || featid > features.size())
			return;
...
int Features::deletePoint(int featid, int pointid)
{
	// Check if feature is valid
	if (featid < 1 || featid > features.size())
		return 0;
...
void Features::setName(int featid, string _name)
{
	// Check if feature is valid
	if (featid < 1 || featid > features.size())
		return;
...
string Features::getName(int featid)
{
	// Check if feature is valid
	if (featid < 1 || featid > features.size())
		return "";
...
void Features::setDescription(int featid, string _desc)
{
	// Check if feature is valid
	if (featid < 1 || featid > features.size())
		return;
...
string Features::getDescription(int featid)
{
	// Check if feature is valid
	if (featid < 1 || featid > features.size())
		return "";
...
void Features::setFeatureClassId(int featid, int fclass)
{
	// Check if feature is valid
	if (featid < 1 || featid > features.size())
		return;
...
int Features::getFeatureClassId(int featid)
{
	// Check if feature is valid
	if (featid < 1 || featid > features.size())
		return -1;
...
void Features::setFeatureType(int featid, int ftype)
{
	// Check if feature is valid
	if (featid < 1 || featid > features.size())
		return;
...
int Features::getFeatureType(int featid)
{
	// Check if feature is valid
	if (featid < 1 || featid > features.size())
		return -1;
...
void Features::setLayer(int featid, int layer)
{
	// Check if feature is valid
	if (featid < 1 || featid > features.size())
		return;
...
int Features::getLayer(int featid)
{
	// Check if feature is valid
	if (featid < 1 || featid > features.size())
		return -1;
...
int Features::getNumPoints(int featid)
{
	// Check if feature is valid
	if (featid < 1 || featid > features.size())
		return -1;
...
// Return class id
int Features::addFeatureClass(string name, string description, int ccolor, int fcolor)
{
	FeatureClass fc;
	fc.name = name;
...
// Return class id
int Features::addFeatureClass(FeatureClass fc)
{
...
void Features::setFeatureClass(int classid, FeatureClass fc)
{
	if (classid < 1 || classid > feature_classes.size())
		return;
...
void Features::calculateFeaturesAttributes()
{
...
void Features::calculateFeatureAttributes(int featid)
{
	if (featid < 1 || featid > features.size())
		return;
...
void Features::calculateCentroid(int featid)
{
	if (featid < 1 || featid > features.size())
		return;

	Feature *df =  &features.at(featid-1);
...
void Features::calculatePerimeter(int featid)
{
	if (featid < 1 || featid > features.size())
		return;
...
void Features::calculateArea(int featid)
{
	if (featid < 1 || featid > features.size())
		return;
...
void Features::checkAllIsOnScreen()
{
	for (int i=1; i<=features.size(); i++)
		checkIsOnScreen(i);
}

void Features::checkIsOnScreen(int featid)
{
	if (featid < 1 || featid > features.size())
			return;
...
int Features::getNearestFeature(double X, double Y, double Z)
{
	if (features.size() < 1)
		return 0;
...
int Features::getNearestPoint(int fid, double X, double Y, double Z)
{
...
void Features::getNearestPoint(double X, double Y, double Z, int& fid, int& pid)
{
	if (features.size() < 1)
		return;
...
//
void Features::convertClassesIdsFromSp165()
{
	int fc, ft;
	for (int i=0; i<features.size(); i++)
	{
...
int Features::convClassFromSp165(int ft, int fc)
{
	int new_fc;
...
int Features::getClassIdToSp165(int new_class)
{
	int old_class;
...
void Features::createClassesFromSp165()
{
...
int Features::loadFeatSp165(char *filename, bool append=false)
{
...
void Features::showFeatures(bool full=false)
{
 ...
 */ //Deprecated
/*
void Features::addFeaturesToPairList(MatchingPointsList *mpl, bool usePolygons = false)
{
	// Search for features
	for (int f=0; f<features.size(); f++)
	{
		Feature *df =  &features.at(f);
 ...
 */
Matrix Features::createPolygonMap(double Xi, double Yi, double Xf, double Yf, double res_x, double res_y)
{
	Matrix polyMap;

	// Calculate feature size
	int f_width, f_height;
	f_width = int(1.0 + floor((Xf - Xi) / res_x));
	f_height = int(1.0 + floor((Yf - Yi) / res_y));

	polyMap.resize(f_height, f_width);
...
void Features::addPolygonToMap(int feat_id, Matrix *map, double Xi, double Yi, double res_x, double res_y)
{
...
Matrix Features::mapPolygon(int feat_id, double res_x, double res_y)
{
	Matrix pmap;
...
// Moving average
double Features::interpolateXYPolygon(int feat_id, double X, double Y, double D0 = -1.0)
{
	if (feat_id < 1 || feat_id > features.size())
		return 0.0;
	...
	if (D0 < 0.0)
	{
		...
		Feature *df = &features.at(feat_id-1);
...
void Features::calculateBoundingBox(int feat_id, double &Xi, double &Yi, double &Xf, double &Yf)
{
	if (feat_id < 1 || feat_id > features.size())
		return;
 ...
 */
double Features::angle2D(double X1, double Y1, double X2, double Y2)
{
...
bool Features::isInside(int feat_id, double X, double Y)
{
	if (feat_id < 1 || feat_id > features.size())
		return false;
...
string Features::getFeaturesToDisplay(int mode)
{
...
void Features::setImagePairSize(int lw, int lh, int rw, int rh)
{
	img_left_width = lw;
	img_left_height = lh;
	img_right_width = rw;
