...
* * * * * * * * * * * *
* @date 10/06/2011
* @version 1.1 - Marcelo Teixeira Silveira & Irving Badolato
*/
...
{
public:
	Feature() { feature_class = feature_type = layer = 1; perimeter = area = 0.0; flag_calc_centroid = 0; is_on_screen = 0; };
	int feature_class, feature_type, layer, is_on_screen;
	double perimeter, area;
	string name, description;
	FeaturePoints centroid;
	bool flag_calc_centroid;
	vector <FeaturePoints> points;
};
...
public:
	...
	Feature getFeature(int feat); // Read only
	Feature* getFeatureLink(int feat);
	...
	//void addFeaturesToPairList(MatchingPointsList *mpl, bool usePolygons); // deprecated
