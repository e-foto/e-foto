	...
	try
	{
		conn = TeDatabaseFactory::make(params);
                ...
                if (conn==0)
                    ...
                    std::cout << "---------conn!=0" << std::endl;
                std::cout << "---------params.database_ : " << params.database_ << std::endl;
                std::cout << "---------params.user_ : " << params.user_ << std::endl;
                std::cout << "---------params.password_ : " << params.password_ << std::endl;
                std::cout << "---------params.host_ : " << params.host_ << std::endl;
                std::cout << "---------params.port_ : " << params.port_ << std::endl;
                //fim dos testes
		if(conn->newDatabase(params.database_, params.user_, params.password_, params.host_, params.port_) == false)
		{

                        std::string errorMessage = conn->errorMessage();
                        std::cout << "Erro: " << errorMessage << std::endl;
			delete conn;
			return false;
		}
	}
...
bool Te3DPersistenceService::addPoint(const std::string& layerName, const TePoint3D& point3D, const std::string& className)
{
	if(conn_ == 0)
	{
            std::cout << "Erro: Conexão não existe" << std::endl;
		return false;
	}

	if(layerExists(layerName) == false)
	{
            std::cout << "Erro: Layer \"" << layerName << "\" não existe" << std::endl;
                return false;
	}
	...
	if(layer == 0)
	{
            std::cout << "Erro: Layer \"" << layerName << "\" não foi capturado" << std::endl;
                return false;
	}
...
bool Te3DPersistenceService::addPolygon(const std::string& layerName, const TePolygon3D& polygon3D, const std::string& className)
{
	...
	if(layerPoints == 0)
	{
            std::cout << "Erro: Layer \"" << layerName << "\"" << std::endl;
		return false;
	}
...
//we first store the 2D part
	...
	{
                std::string errorMessage = conn_->errorMessage();
                std::cout << "Erro1: " << errorMessage << std::endl;
                //conn_->rollbackTransaction();
		return false;
	}
	...
	{
                std::string errorMessage = conn_->errorMessage();
                std::cout << "Erro2: " << errorMessage << std::endl;
                //conn_->rollbackTransaction();
		return false;
	}
	...
	if(layer->saveAttributeTable(table) == false)
        {
                std::string errorMessage = conn_->errorMessage();
                std::cout << "Erro3: " << errorMessage << std::endl;
                //conn_->rollbackTransaction();
                return false;
	}
...
//then we store the 3d part
	for(unsigned int i = 0; i < vecCoords.size(); ++i)
	{
		...
		{
                        std::string errorMessage = conn_->errorMessage();
                        std::cout << "Erro4: " << errorMessage << std::endl;
                        //conn_->rollbackTransaction();
			return false;
		}
	...
	{
                std::string errorMessage = conn_->errorMessage();
                std::cout << "Erro5: " << errorMessage << std::endl;
                //conn_->rollbackTransaction();
		return false;
	}
